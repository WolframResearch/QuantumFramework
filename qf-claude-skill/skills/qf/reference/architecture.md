...
`PauliStabilizerApply[qco, ps]` (in `Stabilizer/PauliStabilizer.m`) is the `Method -> "Stabilizer"` circuit dispatcher: returns `$Failed` (one `::nonclifford`) on a non-Clifford gate, or a `StabilizerFrame`/`Plus` for the P/T boundary. **Since the June 2026 round 2 (`Compiled.m`)** it routes an all-Clifford concrete circuit through a single `Compile`-to-C **bulk gate fold** (`ps["ApplyCircuit", specs]`) over a generator-major packed tableau (~0.2-0.3 µs/gate, flat in n; auto-selects `"C"`/`"WVM"` by compiler availability), falling back to the per-gate fold for anything unencodable. **A user-facing front door to this engine landed at `c3f21a43`:** the multi-gate bracket forms `ps[{spec, ...}]` (list) and `ps[spec, ...]` (variadic) normalize each spec to `gate -> order` (`stabilizerNormalizeSpec`, bare names get a default target) and route the list to `ps["ApplyCircuit", ...]`. They are disambiguated from the property accessor (`ps["X"]`, a `_String`), the integer-list measurement (`ps[{1, 2}]`), and single-gate calls by the `stabilizerGateSpecQ` / `…ListQ` / `…SeqQ` guards (`PauliStabilizer.m:141-168`); the recognized-name catalog `$stabilizerCliffordNames` (`Compiled.m:58`) is a third gate-name list, distinct from the C-kernel `$stabilizerGateCodes` and the frame `sfGate1`/`sfGate2` tables. `ApplyCircuit` returns a **canonical** (rank-3) object, unlike single gates which return packed ones. The closed-form `PauliStabilizer[n]` register constructor and the packed measurement kernels are the other round-2 pieces; `RandomClifford.m` was hardened (`\mathbb{F}_2` inverse + packed matmul; exact-rational Mallows ratio) so `PauliStabilizer["Random"[n]]` no longer blows up near n≈500 or underflows past n≈538. Two pre-existing AG measurement bugs (destabilizer rowsum omitted; non-abelian Pauli-string group) were fixed in passing (`1c524432`/`52d3ea89`/`730abcec`). Full story: `OngoingProjects/Platform Comparison/QF-Stabilizer-Optimization-Report.md`; phase-resolved benchmarks (2026-07-20): kernel within 4-7× of scalar Stim, end-to-end 11-16× (boundary-dependent), ahead of QC.jl at n=1000. The former n>1000 sparse-tableau interpreted fallback was FIXED at `253afdfb` (TargetStructure->"Dense" ctor + densify in packGenRows); compiled fold verified to n=4000 at 0.22-0.50 us/gate.
