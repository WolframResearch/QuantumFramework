...
- **Multi-gate bracket forms** (`PauliStabilizer.m:141-168`, shipped `c3f21a43`): `ps[{spec, ...}]` applies a whole circuit by routing to `ps["ApplyCircuit", ...]` (the compiled engine, canonical output); `ps[spec, spec, ...]` is braceless variadic sugar that forwards to the list form (`ps[spec__] /; stabilizerGateSpecSeqQ[{spec}] := ps[{spec}]`). A `spec` is an arrow `gate -> order`, a bare Clifford name with a default target (one-qubit gate → qubit 1, two-qubit gate → `{1, 2}`; `stabilizerDefaultTarget`), or a call-form (`"P"[θ]`, `SuperDagger["S"|"V"|"T"]`); `stabilizerNormalizeSpec` rewrites every spec to the `gate -> order` shape the engine already accepts. **Disambiguation guards** (`stabilizerGateSpecQ`/`stabilizerGateSpecListQ`/`stabilizerGateSpecSeqQ`): the list form `ps[{"X"}]` is structurally distinct from the single-string property accessor `ps["X"]` (a `_List`, not a `_String`) and from the integer-list measurement `ps[{1, 2}]` (caught by the more specific `{___Integer}` rule); the variadic form's `Length ≥ 2` + gate-membership guard means a single bare gate `ps["H"]` and a single-gate call `ps[gate, target]` stay inert, and an unrecognized token (`ps["Wobble", "Splork"]`) is rejected (the guard is `False`, so the rule does not fire and the call stays unevaluated). Catalogs `$stabilizerCliffordNames` = `{H,S,X,Y,Z,V,T,CNOT,CX,CZ,SWAP}` and `$stabilizerTwoQubitNames` = `{CNOT,CX,CZ,SWAP}` live in `Compiled.m:57-58` and are deliberately distinct from `$stabilizerGateCodes` (the C-kernel encoding, which omits non-Clifford `V`/`T`) and from `sfGate1`/`sfGate2` (the frame conjugation tables). `PauliStabilizer::badgate` is defensive only: the public path never reaches it because `stabilizerGateSpecQ` filters unknown tokens upstream (it fires solely if `stabilizerDefaultTarget` is called directly on a non-string, non-`"P"[_]`, non-`SuperDagger` head). Verified: `AuditMatrix.wlt` TIER 3b + `PauliStabilizer.wlt` TIER 10b (216/216 and 336/336 against the working tree).
...
Superpositions `Σ cᵢ|sᵢ⟩` of stabilizer states (GarMar15 Quipu). `StabilizerFrame[{{c, ps}, ...}]` or `[ps]`, internally `StabilizerFrame[<|"Components" -> {{c, ps}, ...}, "Paulis" -> ...|>]`. Properties: `"Components"`, `"Coefficients"`, `"Stabilizers"`, `"Length"`, `"Qubits"`, `"StateVector"`/`"State"` (materialize), `"InnerProduct"`. Clifford gates distribute over components; `"P"[θ]`/`"T"`/`SuperDagger["T"]` double the frame. `Plus`/`Times`/`Equal` upvalues defined. Note that a frame is produced by applying a non-Clifford gate to a stabilizer (`PauliStabilizer[2]["T", 1]` is a `StabilizerFrame`), so the frame methods below are reached from ordinary `PauliStabilizer` use.

**Multi-gate forms on a frame** (`StabilizerFrame.m:263-266`, shipped `c3f21a43`, mirrors `PauliStabilizer`'s list/variadic forms with the same `stabilizerGateSpecListQ`/`stabilizerGateSpecSeqQ` guards): `f[{spec, ...}]` and `f[spec, spec, ...]`. A frame has no compiled engine, so the list form **folds per gate** through the single-gate handlers (`Fold[#1[stabilizerNormalizeSpec[#2]] &, f, specs]`), each step updating the relating Paulis coherently; the variadic form forwards to the list form. **Companion fix in the same commit:** the single-gate distribute rules were tightened from `f_StabilizerFrame[gate_String, args___]` / `[SuperDagger[gate_String], args___]` to `args : ___Integer` (`StabilizerFrame.m:201, 212`). Without the restriction a trailing **gate name** (`f["H", "S"]`) was captured as `args` and passed to `sfConjugatePauli` as a qubit index, silently corrupting the relating Paulis; with it, the trailing gate falls through to the multi-gate variadic rule instead. Verified parity (`AuditMatrix.wlt` `Audit-Multi-Frame-List-Coherent`/`-Variadic-Coherent`): on `sf = PauliStabilizer[2]["T", 1]`, both `sf[{"H", "S" -> 2}]` and `sf["H", "S" -> 2]` reproduce `sf["H", 1]["S", 2]` to amplitude 0.

**Relating Paulis + phase-coherent materialization (`61bc0e39`, line numbers in this file shifted with the fix, so cite handlers by rule name).** A **gate-built** frame additionally carries a `"Paulis"` key: one relating Pauli per component, encoded `{xbits, zbits, coeff}` with `coeff ∈ {1, -1, I, -I}` (so `{1,1}` on a qubit is Y), component 1's relating Pauli the identity, under the contract `|componentᵢ⟩ = matrix[Pᵢ] . |reference⟩`. Justification: every component of a gate-built frame shares one stabilizer tableau (a Z update leaves it unchanged, a Clifford update transforms it identically), so each component is a definite Pauli on a single reference component; the relative phases are physical but **not** recoverable from the final frame data alone, hence the bookkeeping. Maintained O(n)/gate by the `sfGate1`/`sfGate2`/`sfTimesZ` lookup tables: the Clifford-distribute rule (`f_StabilizerFrame[gate_String, args : ___Integer]`, tightened from `args___` in `c3f21a43`) conjugates every relating Pauli `P -> G.P.G†` via `sfConjugatePauli`; the `"P"[θ]`/`"T"` doubling rule left-multiplies the new branch by `Z_q` via `sfTimesZPauli`; the `Times` upvalue carries `"Paulis"` through. `f["StateVector"]` (the `"StateVector"` handler) now **branches**: with a `"Paulis"` key it materializes the reference component **once** and relates the rest by `sfApplyPauli` (recovering all relative phases, phase-coherent up to a single global phase, same contract as a bare `PauliStabilizer`); without the key (hand-built `StabilizerFrame[{{c, ps}, ...}]`, or any `Plus`-combined frame, where components need not share a tableau) it keeps the prior independent per-component sum, so nothing regresses. **Verified live (working-tree paclet):** `T|+⟩ = {1, e^{iπ/4}}/√2` exactly (the case the old per-component readout swapped to fidelity 0.5); amplitude-exact vs dense `Method -> "Schrodinger"` across 216 random Clifford+T circuits (worst phase-aligned amplitude error 0); `⟨Y⟩` on `T|+⟩` is now `+0.7071` (was sign-flipped to `-0.7071`); manual frames unchanged. Cost ≈ +15% `ByteCount`, +8-12% build time per frame; the `2^k` component-doubling-per-non-Clifford-gate law and the lack of rank compression are unchanged.
