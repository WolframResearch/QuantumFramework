...
for dd in range(num_steps):
    result_circ = trotter_simulation.run(state=result_circ) #Use previous result as start of next step
    state_trotter_circ.append(result_circ)

# Run pulse-level noisy hardware simulation

processor = SCQubits(num_qubits=2, t1=2e5, t2=2e5) #Superconducting cicuits processor
processor.load_circuit(trotter_simulation) #Load circuit into processor
# Since SCQubit are modelled as qutrit, we need three-level systems here for initial condition:
