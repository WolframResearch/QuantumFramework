...
## Part VI: Three Traps Worth Knowing

The first cost me a day, and it is pure Wolfram Language rather than physics. In a `.wls` file or a package, a newline *ends* a syntactically complete expression, so a continuation line that begins with a binary operator is parsed as a separate statement and silently discarded:
...
The result is `10`, not `7`. The second line was parsed as its own statement and thrown away. This only bites at *top level*: inside an unclosed bracket a newline terminates nothing, which is exactly what makes the bug hard to reproduce once you go looking for it. An operator-splitting generator assembled across two lines can quietly lose a whole term this way, and if that term is the dissipator the only visible symptom is a purity stuck at exactly $1$. Put the operator at the *end* of the previous line, or wrap the right-hand side in parentheses.

The second is about the noncommutative algebra. `NonCommutativeAlgebra` takes a *single* association of settings, not a sequence of options: `NonCommutativeAlgebra["Generators" -> ..., "CommutationRelations" -> ...]` fails with `argt`, "called with 3 arguments; 0 or 1 arguments are expected." Wrap the settings in `<|...|>`. And the commutation relations must be entered leading-monomial-positive and must themselves form a Gröbner basis, or the object returns unevaluated rather than throwing; test `ListQ[alg["Generators"]]` to confirm it actually built.

The third: `DSolve` returns its replacement rules in alphabetical order, not in the order you asked for them. Extract by name. Positional extraction on a system like ours hands you $C(t)$ when you think you are holding $V_x(t)$, and both are smooth positive functions that plot beautifully.

All three share a shape, which is why I group them: none produces an error you would notice. In a language this forgiving, the absence of a complaint is not evidence.
