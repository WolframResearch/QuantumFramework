...
- 3.3 [BSc] How do I build the maximally mixed state $I/d$ and a general mixed state?
...
- 6.1 [BSc] How do I verify the Robertson-Schrodinger uncertainty relation for a given state?
- 6.2 [BSc] How do I find a minimum-uncertainty state that saturates the Robertson bound for a chosen pair of observables?
- 6.3 [BSc] How do I compute the classical Shannon entropy $H(p)=-\sum_i p_i\log_2 p_i$ of a measurement distribution?
- 6.4 [BSc] How do I build the three mutually unbiased bases of a qubit (the eigenbases of $X$, $Y$, $Z$)?
- 6.5 [MSc] How do I compute an entropic uncertainty relation between two observables?
- 6.6 [MSc] How do I show that two mutually unbiased bases saturate an entropic uncertainty relation (maximal complementarity)?

## Part 7. Unitary dynamics and pictures
- 7.1 [BSc] How do I build the time-evolution operator $e^{-i\hat H t}$ and solve the Schrodinger equation?
...
- 7.5 [BSc] How do I evolve a state in the Heisenberg picture and check Ehrenfest's theorem?
...
- 7.7 [MSc] How do I derive a quantum speed limit from the time-energy uncertainty relation (Mandelstam-Tamm)?
- 7.8 [MSc] How do I solve a classically driven two-level system in the rotating frame (the Rabi problem)?
- 7.9 [MSc] How do I compute discrete-level transition probabilities under a periodic perturbation by time-dependent perturbation theory (the resonance $\mathrm{sinc}^2$ formula), and recover Fermi's golden rule in the dense-band limit?
- 7.10 [MSc] How do I find a symmetry operator that commutes with $\hat H$, use it as a conserved quantum number, and block-diagonalize $\hat H$ by symmetry sector?
...
- 7.13 [MSc] How do I compute Berry's geometric phase around a closed loop as a discrete Bargmann invariant $\arg\prod_k\langle\psi_k|\psi_{k+1}\rangle$?
...
- 8.4 [MSc] How do I construct the Wigner $D$-matrix for a spin-$j$ rotation?
...
- 9.1 [BSc] How do I build the standard single-qubit gates (X, Y, Z, H, S, T) and rotations $R_n(\theta)$?
...
- 9.6 [MSc] How do I realize a single-qubit unitary as a sequence of $\{H, T\}$ gates (the Solovay-Kitaev idea)?
...
- 10.2 [BSc] How do I compose a circuit, apply it, and read off its overall unitary?
...
- 11.8 [MSc] How do I construct an entanglement witness that detects a given entangled state?
- 11.9 [MSc] How do I decide LOCC convertibility of pure states by Nielsen's majorization theorem?
...
- 11.12 [MSc] How do I construct a $3\times3$ (or $2\times4$) PPT-entangled (bound) state, and why can none exist for two qubits (Horodecki)?
...
- 12.7 [MSc] How do I find the optimal state-discrimination measurement and the Helstrom bound?
- 12.8 [MSc] How do I verify Gleason's frame-function constraint for a qutrit ($d\geq3$), and why does the qubit ($d=2$) need the POVM (Busch) version?
...
- 13.7 [MSc] How do I construct the complementary channel and relate it to the environment?
- 13.8 [MSc] How do I test whether a channel is unital, and what does that mean for the Bloch ball?
- 13.9 [MSc] How do I compose channels and test divisibility (Markovianity)?
...
- 13.15 [MSc] How do I bound the distinguishability of two channels by the diamond norm?
...
- 14.4 [MSc] How do I compute the coherent information of a state through a channel?
- 14.5 [MSc] How do I verify the data-processing inequality under a channel?
- 14.6 [MSc] How do I compute the Holevo bound and the accessible information of an ensemble?
...
- 14.8 [MSc] How do I quantify the coherence of a state in a fixed basis (a resource measure)?
...
- 17.4 [MSc] How do I estimate average gate fidelity by randomized benchmarking?
...
- 19.2 [MSc] How do I describe the Pauli and Clifford groups as groups (order, generators, normalizer)?
...
- 20.4 [MSc] How do I build a CSS code from two classical codes, with the Steane $[[7,1,3]]$ as the worked instance?
- 20.5 [MSc] How do I build the five-qubit perfect code's stabilizers and correct an arbitrary single-qubit error with it (the smallest, non-CSS, code, saturating the quantum Hamming bound)?
- 20.6 [MSc] How do I compute a code's distance?
- 20.7 [MSc] How do I identify a code's logical operators?
- 20.8 [MSc] How do I lay out a surface-code patch and read its stabilizers and logical qubit?
- 20.9 [MSc] How do I demonstrate a transversal (fault-tolerant) logical gate and explain the threshold theorem?
...
- 21.5 [MSc] How do I estimate the stabilizer rank of a magic state?
- 21.6 [MSc] How do I run a magic-state distillation round and watch the output fidelity improve?
...
- 22.2 [MSc] How do I find the ground state and the energy gap of a Heisenberg or XY chain?
- 22.3 [MSc] How do I locate a finite-size quantum phase transition by the gap and an order parameter?
- 22.4 [MSc] How do I compute the entanglement entropy of a block and observe area-law scaling?
- 22.5 [MSc] How do I write a small state as a matrix product state and read its bond dimension?
- 22.6 [MSc] How do I evolve a spin chain and observe a Lieb-Robinson light cone?
...
- 23.6 [MSc] How do I generalize Grover to amplitude amplification on an arbitrary initial state?
...
- 23.12 [MSc] How do I implement a discrete-time quantum walk on a finite graph?
- 23.13 [BSc] VQE: how do I find a ground-state energy variationally?
...
- 23.16 [MSc] How do I exhibit a barren plateau (the gradient variance vanishing with system size)?
...
- 25.5 [MSc] PR boxes: how do I show quantum mechanics is more nonlocal than classical but less than no-signalling allows?
...
|------|-------|-------|-----|-----|
| 1 | Pure states and the Born rule | 9 | 9 | 0 |
| 2 | Observables, spectra, approximation | 13 | 9 | 4 |
| 3 | States as density operators | 8 | 7 | 1 |
| 4 | Composite systems: tensor product, partial trace | 2 | 2 | 0 |
| 5 | Projective measurement | 3 | 3 | 0 |
| 6 | Uncertainty and incompatibility | 6 | 4 | 2 |
| 7 | Unitary dynamics and pictures | 16 | 6 | 10 |
| 8 | Spin and angular momentum | 7 | 3 | 4 |
| 9 | Single-qubit operations, SU(2) | 6 | 4 | 2 |
| 10 | Elementary multi-qubit gates and circuits | 3 | 3 | 0 |
| 11 | Composite systems and entanglement | 13 | 5 | 8 |
| 12 | Mixed states: distinguishability, thermal | 8 | 4 | 4 |
...
| 20 | Error correction | 9 | 1 | 8 |
| 21 | Discrete phase space, magic | 6 | 0 | 6 |
| 22 | Finite many-body | 6 | 0 | 6 |
| 23 | Algorithms | 16 | 6 | 10 |
| 24 | Communication | 5 | 3 | 2 |
| 25 | Foundations | 11 | 4 | 7 |
| **Total** | | **194** | **83** | **111** |

A two-semester reading worked strictly in order. The 83 [BSc] items form a self-contained first
course in finite-dimensional quantum theory and quantum information, each answerable from earlier
items; the 111 [MSc] items extend it to a graduate course covering open systems, error correction,
