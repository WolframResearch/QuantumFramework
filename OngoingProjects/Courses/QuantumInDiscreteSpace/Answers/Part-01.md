...
**WL** : square the moduli of the normalized amplitudes, mapping `FullSimplify` across the two
components so each collapses to a single ratio.

```wl
FullSimplify /@ Abs[Normalize[{\[Alpha], \[Beta]}]]^2
```
