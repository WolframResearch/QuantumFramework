...
> [DSolve](https://reference.wolfram.com/language/ref/DSolve.html)[*eqn*] — solves a differential equation `*eqn*`.
> [DSolve](https://reference.wolfram.com/language/ref/DSolve.html)[*eqn*,*u*,*x*] — solves a differential equation for the function `*u*`, with independent variable `*x*`.
> [DSolve](https://reference.wolfram.com/language/ref/DSolve.html)[*eqn*,*u*,{*x*,*x*_*min*,*x*_*max*}] — solves a differential equation for `*x*` between `*x*_*min*` and `*x*_*max*`.
> [DSolve](https://reference.wolfram.com/language/ref/DSolve.html)[{*eqn*_1,*eqn*_2,…},{*u*_1,*u*_2,…},…] — solves a list of differential equations.
> [DSolve](https://reference.wolfram.com/language/ref/DSolve.html)[*eqn*,*u*,{*x*_1,*x*_2,…}] — solves a partial differential equation.
> [DSolve](https://reference.wolfram.com/language/ref/DSolve.html)[*eqn*,*u*,{*x*_1,*x*_2,…}∈Ω] — solves the partial differential equation `*eqn*` over the region `Ω`.

## Details and Options

[DSolve](https://reference.wolfram.com/language/ref/DSolve.html) can solve ordinary differential equations (ODEs), partial differential equations (PDEs), differential algebraic equations (DAEs), delay differential equations (DDEs), integral equations, integro-differential equations, and hybrid differential equations.

Different classes of equations solvable by [DSolve](https://reference.wolfram.com/language/ref/DSolve.html) include:
...
The output from [DSolve](https://reference.wolfram.com/language/ref/DSolve.html) is controlled by the form of the dependent function `*u*` or `*u*[*x*]`:
...
The dependent variable in [DSolve](https://reference.wolfram.com/language/ref/DSolve.html)[*eqn*] is inferred from `*eqn*` and may be specified either explicitly as `*u*[*x*]` or as a pure function `*u*` for autonomous equations.

[DSolve](https://reference.wolfram.com/language/ref/DSolve.html) can give implicit solutions in terms of [Solve](https://reference.wolfram.com/language/ref/Solve.html).

[DSolve](https://reference.wolfram.com/language/ref/DSolve.html) can give solutions that include [Inactive](https://reference.wolfram.com/language/ref/Inactive.html) sums and integrals that cannot be carried out explicitly. Variables `K[1]`, `K[2]`, `…` are used in such cases.
...
[N](https://reference.wolfram.com/language/ref/N.html)[DSolve[…]] calls [NDSolve](https://reference.wolfram.com/language/ref/NDSolve.html) or [ParametricNDSolve](https://reference.wolfram.com/language/ref/ParametricNDSolve.html) for differential equations that cannot be solved symbolically.
...
(* Output *)
{{y[x]->-(1)/(2) a ℯ^-x (-1+ℯ^x Cos[x]-ℯ^x Sin[x])}}
```
...
(* Output *)
{{y->Function[{x},-(1)/(2) a ℯ^-x (-1+ℯ^x Cos[x]-ℯ^x Sin[x])]}}
```
...
(* Output *)
{(1)/(4) a ℯ^(-2 x) (a-2 ℯ^x (-1+a Cos[x]-a Sin[x])+ℯ^(2 x) (a+2 Cos[x]-2 Sin[x]-a Sin[2 x]))}
```

Specify only the first argument of [DSolve](https://reference.wolfram.com/language/ref/DSolve.html):
...
Solve the same equation with an implicit independent variable $x$:
...
```wolfram
Plot[{y[x]/.sol/.1->2,y[x]/.sol/.1->3},{x,1,4}]
```
...
```wolfram
sol=DSolve[{y'[x]==-3y[x]^2,y[1]==2},y[x],x]
(* Output *)
{{y[x]->(2)/(-5+6 x)}}
```
...
sol=DSolve[eqns,y,x]
(* Output *)
{{y->Function[{x},(1)/(2) (6 ℯ^x+a ℯ^x-a Cos[x]+a Sin[x])]}}
```
...
```wolfram
DSolve[{y''[x]+4y[x]==7},y[x],x]
(* Output *)
{{y[x]->(7)/(4)+1 Cos[2 x]+2 Sin[2 x]}}
```

Particular solution:

```wolfram
DSolve[{y''[x]+4y[x]==7,y[0]==1,y'[0]==2},y[x],x]
(* Output *)
...
```wolfram
sol=DSolve[eqns,{y,z},x]
(* Output *)
{{y->Function[{x},(ℯ^(-Sqrt[1-a] x) (-1+Sqrt[1-a]+4 a+ℯ^(2 Sqrt[1-a] x)+Sqrt[1-a] ℯ^(2 Sqrt[1-a] x)-4 a ℯ^(2 Sqrt[1-a] x)))/(2 Sqrt[1-a])],z->Function[{x},(ℯ^(-Sqrt[1-a] x) (3+4 Sqrt[1-a]-3 ℯ^(2 Sqrt[1-a] x)+4 Sqrt[1-a] ℯ^(2 Sqrt[1-a] x)))/(2 Sqrt[1-a])]}}
```

Plot the solution:

```wolfram
ParametricPlot[Evaluate[Table[{y[x],z[x]}/.sol,{a,3,8}]],{x,0,10}]
(* Output *)
...
(* Output *)
{{y[x]->1 Cos[x]+2 Sin[x],z[x]->2 Cos[x]-1 Sin[x]}}
```

Solve a partial differential equation:

```wolfram
eqn=D[u[x,y],x]+3D[u[x,y],y]+u[x,y]==1;
```

```wolfram
sol=DSolve[eqn,u,{x,y}]
(* Output *)
{{u->Function[{x,y},ℯ^-x (ℯ^x+1[-3 x+y])]}}
```
...
sol/.1->Sin
(* Output *)
{{u->Function[{x,y},ℯ^-x (ℯ^x+Sin[-3 x+y])]}}
```

Plot the solution:

```wolfram
Plot3D[u[x,y]/.%,{x,-3,3},{y,-3,3},PlotRange->All]
(* Output *)
...
```wolfram
DSolve[{D[u[x,t],{t,2}]==D[u[x,t],{x,2}]}]
(* Output *)
{{u[x,t]->1[t-x]+2[t+x]}}
```
...
```wolfram
DSolve[y''[x]-4y[x]==0,y[x],x]
(* Output *)
{{y[x]->ℯ^(2 x) 1+ℯ^(-2 x) 2}}
```

```wolfram
DSolve[y''[x]-4y[x]==0,y[x],x,GeneratedParameters->f]
(* Output *)
{{y[x]->ℯ^(2 x) f[1]+ℯ^(-2 x) f[2]}}
```

Solve a delay differential equation:

```wolfram
sol=DSolve[{x'[t]==x[t-1]^2,x[t/;t<=0]==a t},x[t],{t,0,2}]
(* Output *)
{{x[t]->{, {{a t, t<=0}, {(1)/(3) (3 a^2 t-3 a^2 t^2+a^2 t^3), 0<t<=1}, {(1)/(126) (42 a^2-163 a^4+686 a^4 t-1176 a^4 t^2+1064 a^4 t^3-553 a^4 t^4+168 a^4 t^5-28 a^4 t^6+2 a^4 t^7), 1<t<=2}, {Indeterminate, True}}}}}
```

Plot the solution for different values of the parameter:

```wolfram
Plot[Evaluate[Table[x[t]/.sol,{a,1,5}]],{t,-1,2}]
```
...
```wolfram
sol=DSolve[{y''[t]==-981/100,y[0]==5,y'[0]==0,WhenEvent[y[t]==0,y'[t]->-(7/10)y'[t]]},y[t],{t,0,4}]
(* Output *)
{{y[t]->{, {{5-(981 t^2)/(200), 0<=t<=(10 Sqrt[(10)/(109)])/(3)}, {(3)/(200) (-800+34 Sqrt[1090] t-327 t^2), (10 Sqrt[(10)/(109)])/(3)<t<=8 Sqrt[(10)/(109)]}, {(3 (-13520+289 Sqrt[1090] t-1635 t^2))/(1000), 8 Sqrt[(10)/(109)]<t<=(169 Sqrt[(2)/(545)])/(3)}, {(-687154+11169 Sqrt[1090] t-49050 t^2)/(10000), (169 Sqrt[(2)/(545)])/(3)<t<=4}, {Indeterminate, True}}}}}
```
...
Apply [N](https://reference.wolfram.com/language/ref/N.html)[DSolve[...]] to invoke [NDSolve](https://reference.wolfram.com/language/ref/NDSolve.html) if symbolic solution fails:

```wolfram
DSolve[{y''[x]+x^2y'[x]+Sin[y[x]]==1,y[0]==1,y'[0]==1},y,{x,0,1}]
(* Output *)
DSolve[{Sin[y[x]]+x^2 y^′[x]+y^′′[x]==1,y[0]==1,y^′[0]==1},y,{x,0,1}]
```
...
```wolfram
Table[y[x]/.%[[1]],{x,0,1,0.2}]
(* Output *)
{1.,1.2023684799796817,1.4056491387744303,1.6029458197480377,1.78648276971198,1.948817358023112}
```
...
```wolfram
DSolve[{x^′′[t]==5,x[0]==3,x^′[0]==7},x[t],t]
(* Output *)
{{x[t]->3+(7) t+((5)/(2)) t^2}}
```
...
```wolfram
DSolve[y'[x]+x y[x]==Cos[x],y[x],x]
(* Output *)
{{y[x]->ℯ^(-(x^2)/(2)) 1+(1)/(2) ℯ^((1)/(2)-(x^2)/(2)) Sqrt[(π)/(2)] (Erfi[(-ⅈ+x)/(Sqrt[2])]+Erfi[(ⅈ+x)/(Sqrt[2])])}}
```
...
(* Output *)
{{y->Function[{x},(Sqrt[3] AiryAi[x]-AiryBi[x]-3^(2/3) AiryAi[x] AiryBi[9] Gamma[(2)/(3)]+3^(2/3) AiryAi[9] AiryBi[x] Gamma[(2)/(3)])/(Sqrt[3] AiryAi[9]-AiryBi[9])]}}
```
...
(* Output *)
{{y[x]->ℯ^(-2 x) 2 Cos[x]+ℯ^(-2 x) 1 Sin[x]}}
```

Cauchy-Euler equation:

```wolfram
DSolve[x^2y''[x]+4x y'[x]+7y[x]==0,y[x],x]
(* Output *)
{{y[x]->(2 Cos[(1)/(2) Sqrt[19] Log[x]])/(x^(3/2))+(1 Sin[(1)/(2) Sqrt[19] Log[x]])/(x^(3/2))}}
```

Second-order equation with variable coefficients, solved in terms of elementary functions:

```wolfram
DSolve[x y''[x]+2y'[x]-x y[x]==Sin[x],y,x]
(* Output *)
{{y->Function[{x},(ℯ^-x 1)/(x)+(ℯ^x 2)/(2 x)-(Sin[x])/(2 x)]}}
```
...
(* Output *)
{{y->Function[{x},AiryAi[x] 1+AiryBi[x] 2]}}
```

Spheroidal equation:

```wolfram
DSolve[(1-x^2)y''[x]-2x y'[x]+(49-49x^2-16/(1-x^2)+SpheroidalEigenvalue[5,4,7])y[x]==0,y,x]
(* Output *)
{{y->Function[{x},1 SpheroidalPS[5,4,7,x]+2 SpheroidalQS[5,4,7,x]]}}
```

Equations with nonrational coefficients:

```wolfram
DSolve[y''[x]+(8+3Cos[x]^2)y[x]==0,y,x]
(* Output *)
{{y->Function[{x},1 MathieuC[(19)/(2),-(3)/(4),x]+2 MathieuS[(19)/(2),-(3)/(4),x]]}}
```

```wolfram
DSolve[y''[x]+(9+Sech[x]^2)y[x]==0,y,x]
(* Output *)
{{y->Function[{x},1 LegendreP[(1)/(2) (-1+Sqrt[5]),3 ⅈ,Tanh[x]]+2 LegendreQ[(1)/(2) (-1+Sqrt[5]),3 ⅈ,Tanh[x]]]}}
```

```wolfram
DSolve[y''[x]==Exp[-x]y[x],y,x]
(* Output *)
{{y->Function[{x},BesselI[0,2 Sqrt[ℯ^-x]] 1+2 BesselK[0,2 Sqrt[ℯ^-x]] 2]}}
```

Higher-order equations:

```wolfram
DSolve[y'''[x]+4y'[x]==5y[x],y,x]
(* Output *)
{{y->Function[{x},ℯ^x 3+ℯ^(-x/2) 2 Cos[(Sqrt[19] x)/(2)]+ℯ^(-x/2) 1 Sin[(Sqrt[19] x)/(2)]]}}
```

Solution in terms of hypergeometric functions:

```wolfram
DSolve[3x^3y''''[x]+19x^2y'''[x]+(2x-3x^2)y''[x]+(2-9x)y'[x]-3y[x]==0,y,x]
(* Output *)
{{y->Function[{x},-(-1)^(2/3) x^(5/3) 2 HypergeometricPFQ[{(8)/(3)},{(11)/(3)-Sqrt[5],(11)/(3)+Sqrt[5]},x]+(-1)^(-1-Sqrt[5]) x^(-1-Sqrt[5]) 3 HypergeometricPFQ[{-Sqrt[5]},{1-2 Sqrt[5],-(5)/(3)-Sqrt[5]},x]+(-1)^(-1+Sqrt[5]) x^(-1+Sqrt[5]) 4 HypergeometricPFQ[{Sqrt[5]},{-(5)/(3)+Sqrt[5],1+2 Sqrt[5]},x]+1 HypergeometricPFQ[{1,1},{-(2)/(3),2-Sqrt[5],2+Sqrt[5]},x]]}}
```

Fourth-order equation solved in terms of Kelvin functions:

```wolfram
DSolve[(525+x^4)y[x]-51(-x y'[x]+x^2y''[x])+2x^3y'''[x]+x^4y''''[x]==0,y,x]
(* Output *)
{{y->Function[{x},4 KelvinBei[5,x]+3 KelvinBer[5,x]+2 KelvinKei[5,x]+1 KelvinKer[5,x]]}}
```

Linear equation with `*q*`-rational coefficients:

```wolfram
DSolve[((-1+2^x) y^(4)[x])/(1+2^x)+2^x y[x]==0,y,x]
(* Output *)
{{y->Function[{x},4 <|head -> DifferentialRoot, big -> {{(1+x) y[x]+(-1+x) Log[2]^4 y^′[x]+7 (-1+x) x Log[2]^4 y^′′[x]+6 (-1+x) x^2 Log[2]^4 y^(3)[x]+(-1+x) x^3 Log[2]^4 y^(4)[x]==0}, {y[2]==0}, {y^′[2]==0}, {y^′′[2]==0}, {y^(3)[2]==1}}, small -> y^(4)[x], branchcuts -> Re[x]<=0&&Im[x]==0|>[2^x]+3 <|head -> DifferentialRoot, big -> {{(1+x) y[x]+(-1+x) Log[2]^4 y^′[x]+7 (-1+x) x Log[2]^4 y^′′[x]+6 (-1+x) x^2 Log[2]^4 y^(3)[x]+(-1+x) x^3 Log[2]^4 y^(4)[x]==0}, {y[2]==0}, {y^′[2]==0}, {y^′′[2]==1}, {y^(3)[2]==0}}, small -> y^(4)[x], branchcuts -> Re[x]<=0&&Im[x]==0|>[2^x]+2 <|head -> DifferentialRoot, big -> {{(1+x) y[x]+(-1+x) Log[2]^4 y^′[x]+7 (-1+x) x Log[2]^4 y^′′[x]+6 (-1+x) x^2 Log[2]^4 y^(3)[x]+(-1+x) x^3 Log[2]^4 y^(4)[x]==0}, {y[2]==0}, {y^′[2]==1}, {y^′′[2]==0}, {y^(3)[2]==0}}, small -> y^(4)[x], branchcuts -> Re[x]<=0&&Im[x]==0|>[2^x]+1 <|head -> DifferentialRoot, big -> {{(1+x) y[x]+(-1+x) Log[2]^4 y^′[x]+7 (-1+x) x Log[2]^4 y^′′[x]+6 (-1+x) x^2 Log[2]^4 y^(3)[x]+(-1+x) x^3 Log[2]^4 y^(4)[x]==0}, {y[2]==1}, {y^′[2]==0}, {y^′′[2]==0}, {y^(3)[2]==0}}, small -> y^(4)[x], branchcuts -> Re[x]<=0&&Im[x]==0|>[2^x]]}}
```

Higher-order inhomogeneous equation with constant coefficients:

```wolfram
DSolve[y^(4)[x]+5y'''[x]+2y[x]==x E^3x Cos[3x],y[x],x]
(* Output *)
{{y[x]->ℯ^(x <|icon -> Root, small -> "-4.98", approx -> -4.983843893638884, interp -> Root[2, +, 5, #, ^, 3, +, #, ^, 4, &, 1, 0], head -> Root, big -> 2+5 #1^3+#1^4&, degree -> 4, shortDegree -> 4, number -> 1|>) 1+ℯ^(x <|icon -> Root, small -> "-0.780", approx -> -0.7796388260821075, interp -> Root[2, +, 5, #, ^, 3, +, #, ^, 4, &, 2, 0], head -> Root, big -> 2+5 #1^3+#1^4&, degree -> 4, shortDegree -> 4, number -> 2|>) 2+ℯ^(x <|icon -> Root, small -> "0.382"-"0.607" ⅈ, approx -> Complex[0.3817413598604956, -0.6074494163930674], interp -> Root[2, +, 5, #, ^, 3, +, #, ^, 4, &, 3, 0], head -> Root, big -> 2+5 #1^3+#1^4&, degree -> 4, shortDegree -> 4, number -> 3|>) 3+ℯ^(x <|icon -> Root, small -> "0.382"+"0.607" ⅈ, approx -> Complex[0.3817413598604956, 0.6074494163930674], interp -> Root[2, +, 5, #, ^, 3, +, #, ^, 4, &, 4, 0], head -> Root, big -> 2+5 #1^3+#1^4&, degree -> 4, shortDegree -> 4, number -> 4|>) 4-(1)/(22404634562)ℯ^(3 x) (-26914788 Cos[3 x]+31328936 x Cos[3 x]-8230653 Sin[3 x]-14288535 x Sin[3 x])}}
```
...
(* Output *)
{{y->Function[{x},1+(1)/(2) (2 Sqrt[1+4 x]-2 ArcTanh[Sqrt[1+4 x]]-Log[x])]},{y->Function[{x},1+(1)/(2) (-2 Sqrt[1+4 x]+2 ArcTanh[Sqrt[1+4 x]]-Log[x])]}}
```
...
(* Output *)
Solve[(ℯ^(-x+(1)/(y[x])))/(x)+1+ExpIntegralEi[-x+(1)/(y[x])]==0,y[x]]
```

Homogeneous equation:

```wolfram
DSolve[y'[x]-Sqrt[(y[x]/x)]==y[x]/x,y,x]
(* Output *)
{{y->Function[{x},(1)/(4) (x 1^2+2 x 1 Log[x]+x Log[x]^2)]}}
```
...
(* Output *)
{{y->Function[{x},6^(1/3) WeierstrassP[(x+1)/(6^(1/3)),{-2 6^(1/3),2}]]}}
```

Solution in terms of hyperbolic functions:

```wolfram
DSolve[y''[x] ==3y[x] y'[x]+(3y[x]^2+4y[x]+1),y[x],x][[1]]//Simplify
(* Output *)
{y[x]->((ⅈ Sqrt[6] Sqrt[ℯ^(-2 x)] Sqrt[1]-6 2) Cosh[Sqrt[(3)/(2)] Sqrt[ℯ^(-2 x)] Sqrt[1]]+(-3 ⅈ+2 Sqrt[6] Sqrt[ℯ^(-2 x)] Sqrt[1] 2) Sinh[Sqrt[(3)/(2)] Sqrt[ℯ^(-2 x)] Sqrt[1]])/(6 2 Cosh[Sqrt[(3)/(2)] Sqrt[ℯ^(-2 x)] Sqrt[1]]+3 ⅈ Sinh[Sqrt[(3)/(2)] Sqrt[ℯ^(-2 x)] Sqrt[1]])}
```

Solve a nonlinear boundary value problem:

```wolfram
DSolve[{y''[x]==y[x]^3,y[0]==5,y'[0]==25/Sqrt[2]},y[x],x]
(* Output *)
{{y[x]->-(10)/(-2+5 Sqrt[2] x)}}
```
...
```wolfram
DSolve[2 x Sqrt[(1-x)/(1+x)] (1+x) ArcSech[x]^2-ℯ^(ℯ^((f[x])/(ArcSech[x]))+(f[x])/(ArcSech[x])) f[x]-ℯ^(ℯ^((f[x])/(ArcSech[x]))+(f[x])/(ArcSech[x])) x Sqrt[(1-x)/(1+x)] (1+x) ArcSech[x] f^′[x]==0,f[x],x]
(* Output *)
{{f[x]->ArcSech[x] Log[Log[2 x+1]]}}
```
...
```wolfram
DSolve[{y'[x]==x^2y[x],z'[x]==5z[x]}, {y,z},x]
(* Output *)
{{y->Function[{x},ℯ^((x^3)/(3)) 1],z->Function[{x},ℯ^(5 x) 2]}}
```
...
(* Output *)
{{y->Function[{x},(1)/(2) (3 ℯ^(x/2) Cos[(Sqrt[3] x)/(2)]-Cos[(Sqrt[3] x)/(2)]^2-2 ℯ^(x/2) Sin[(Sqrt[3] x)/(2)]+Sqrt[3] ℯ^(x/2) Sin[(Sqrt[3] x)/(2)]-Sin[(Sqrt[3] x)/(2)]^2)],z->Function[{x},(1)/(2) (2 ℯ^(x/2) Cos[(Sqrt[3] x)/(2)]-Sqrt[3] ℯ^(x/2) Cos[(Sqrt[3] x)/(2)]+Sqrt[3] Cos[(Sqrt[3] x)/(2)]^2+3 ℯ^(x/2) Sin[(Sqrt[3] x)/(2)]+Sqrt[3] Sin[(Sqrt[3] x)/(2)]^2)]}}
```
...
```wolfram
DSolve[{y^′[x]==-(4 y[x])/(x)+(4 z[x])/(x),z^′[x]==((4)/(x)-(x)/(4)) y[x]-(4 z[x])/(x)},{y[x],z[x]},x]
(* Output *)
{{y[x]->(BesselJ[4,x] 1)/(x^4)+(BesselY[4,x] 2)/(x^4),z[x]->((BesselJ[3,x])/(8 x^3)-(BesselJ[5,x])/(8 x^3)) 1+((BesselY[3,x])/(8 x^3)-(BesselY[5,x])/(8 x^3)) 2}}
```

Nonlinear system:

```wolfram
DSolve[ {y'[x]==Exp[z[x]]+1,z'[x]==y[x]-x},{y,z},x]//Quiet
(* Output *)
{{z->Function[{x},Log[1+1 Tan[(1)/(2) (Sqrt[2] x Sqrt[1]+2 Sqrt[2] Sqrt[1] 2)]^2]],y->Function[{x},x+Sqrt[2] Sqrt[1] Tan[(1)/(2) (Sqrt[2] x Sqrt[1]+2 Sqrt[2] Sqrt[1] 2)]]}}
```
...
```wolfram
DSolve[x'[t]==m.x[t],x[t]∈Vectors[2],t]
(* Output *)
{{x[t]->{ℯ^t (-2+3 ℯ^t) 1-6 ℯ^t (-1+ℯ^t) 2,ℯ^t (-1+ℯ^t) 1-ℯ^t (-3+2 ℯ^t) 2}}}
```
...
```wolfram
Plot[Evaluate[Flatten[x[t]/.sol[[1]]]],{t,0,10}]
```
...
```wolfram
DSolve[x'[t]-m.x[t]==g,Element[x[t],Vectors[2]],t]
(* Output *)
{{x[t]->{(1)/(6) ℯ^(-Sqrt[3] t) (3-2 Sqrt[3]+3 ℯ^(2 Sqrt[3] t)+2 Sqrt[3] ℯ^(2 Sqrt[3] t)) 1-(ℯ^(-Sqrt[3] t) (-1+ℯ^(2 Sqrt[3] t)) 2)/(2 Sqrt[3])+ℯ^(-2 t) ((991)/(1352)-(27 t)/(52)) Cos[3 t]+ℯ^(-2 t) (-(209)/(676)+(9 t)/(26)) Sin[3 t],(ℯ^(-Sqrt[3] t) (-1+ℯ^(2 Sqrt[3] t)) 1)/(2 Sqrt[3])-(1)/(6) ℯ^(-Sqrt[3] t) (-3-2 Sqrt[3]-3 ℯ^(2 Sqrt[3] t)+2 Sqrt[3] ℯ^(2 Sqrt[3] t)) 2+ℯ^(-2 t) (-(105)/(169)-(29 t)/(26)) Cos[3 t]+ℯ^(-2 t) ((833)/(1352)-(9 t)/(52)) Sin[3 t]}}}
```

Solve a coupled system of linear and nonlinear ODEs:

```wolfram
DSolve[{y^′[x]==-(4 y[x])/(x)+(4 z[x])/(x),z^′[x]==((4)/(x)-(x)/(4)) y[x]-(4 z[x])/(x),y1^′[x]==1+ℯ^z1[x],z1^′[x]==-x+y1[x],w1'[x]+2 E^w2[x]==0,w2'[x]==-w1[x]},{y[x],z[x],y1[x],z1[x],w1[x],w2[x]},x]//Quiet
(* Output *)
{{y[x]->(BesselJ[4,x] 1)/(x^4)+(BesselY[4,x] 2)/(x^4),z[x]->((BesselJ[3,x]-BesselJ[5,x]) 1)/(8 x^3)+((BesselY[3,x]-BesselY[5,x]) 2)/(8 x^3),z1[x]->Log[3+3 Tan[(1)/(2) (Sqrt[2] x Sqrt[3]+2 Sqrt[2] 2 Sqrt[3])]^2],y1[x]->x+Sqrt[2] Sqrt[3] Tan[(1)/(2) (Sqrt[2] x Sqrt[3]+2 Sqrt[2] 2 Sqrt[3])],w2[x]->Log[5+5 Tan[x Sqrt[5]-2 2 Sqrt[5]]^2],w1[x]->-2 Sqrt[5] Tan[x Sqrt[5]-2 2 Sqrt[5]]}}
```

Solve a Fuchsian system of linear ODEs:

```wolfram
DSolve[{y1^′[x]==(y2[x])/((-(2)/(3)+(ⅈ)/(2))+x),y2^′[x]==(y3[x])/(10 ((-4-6 ⅈ)+x)),y3^′[x]==0},{y1[x],y2[x],y3[x]},x]
(* Output *)
{{y1[x]->1+(1)/(10) 2 GeneralizedPolyLog[{(2)/(3)-(ⅈ)/(2),4+6 ⅈ},x]+3 Log[1-((24)/(25)+(18 ⅈ)/(25)) x],y2[x]->3+(1)/(10) 2 Log[1-((1)/(13)-(3 ⅈ)/(26)) x],y3[x]->2}}
```
...
```wolfram
DSolve[Join[Thread[D[{y1[x],y2[x],y3[x]},x]==m.{y1[x],y2[x],y3[x]}],{y1[0]==1,y2[0]==1,y3[0]==1}],{y1[x],y2[x],y3[x]},x]
(* Output *)
{{y1[x]->1-(35 x)/(3)-x^2-(x^3)/(9)+(x (-96+24 x+4 x^2+x^3))/(3 (-2+x))+16 GeneralizedPolyLog[{2,-ⅈ},x]+16 GeneralizedPolyLog[{2,ⅈ},x]+(192)/(5) Log[1-(x)/(2)]-((11)/(5)-(223 ⅈ)/(30)) Log[1-ⅈ x]-(8 Log[1-ⅈ x])/(-2+x)+6 x Log[1-ⅈ x]+x^2 Log[1-ⅈ x]+(1)/(6) x^3 Log[1-ⅈ x]-((11)/(5)+(223 ⅈ)/(30)) Log[1+ⅈ x]-(8 Log[1+ⅈ x])/(-2+x)+6 x Log[1+ⅈ x]+x^2 Log[1+ⅈ x]+(1)/(6) x^3 Log[1+ⅈ x],y2[x]->1+(1)/(2) (Log[1-ⅈ x]+Log[1+ⅈ x]),y3[x]->1}}
```
...
```wolfram
DSolve[{2 y'[x]+z'[x]==4y[x]+x,y[x]-z[x]==1},{y[x],z[x]},x]
(* Output *)
{{y[x]->(1)/(3)+6 ((1)/(288) (-25-12 x)+ℯ^(4 x/3) 1),z[x]->-(2)/(3)+6 ((1)/(288) (-25-12 x)+ℯ^(4 x/3) 1)}}
```

Solve a boundary value problem:

```wolfram
DSolve[{y'[x]-4z[x]==Cos[x], y[x]+z[x]==1/2, y[Pi/2]==1/2},{y,z},x]
(* Output *)
{{y->Function[{x},(1)/(34) ℯ^(-4 x) (-2 ℯ^(2 π)+17 ℯ^(4 x)+8 ℯ^(4 x) Cos[x]+2 ℯ^(4 x) Sin[x])],z->Function[{x},-(1)/(17) ℯ^(-4 x) (-ℯ^(2 π)+4 ℯ^(4 x) Cos[x]+ℯ^(4 x) Sin[x])]}}
```

```wolfram
Plot[Evaluate[{y[x],z[x],y[x]+z[x]}/.%], {x,2,16}]
```
...
```wolfram
DSolve[{x'[t]==x[t]+2y[t],y[t]==x[t]+2^t,x[t]+z[t]==0},{x[t],y[t],z[t]},t]
(* Output *)
{{x[t]->(1)/(8) (-ℯ^(3 t) 1+(16 ℯ^(3 t+t (-3+Log[2])))/(-3+Log[2])),y[t]->2^t+(1)/(8) (-ℯ^(3 t) 1+(16 ℯ^(3 t+t (-3+Log[2])))/(-3+Log[2])),z[t]->(1)/(8) (ℯ^(3 t) 1-(16 ℯ^(3 t+t (-3+Log[2])))/(-3+Log[2]))}}
```
...
```wolfram
Plot[Evaluate[Table[x[t]/.sol[[1]],{a,1,5}]],{t,-1,2}]
```
...
```wolfram
sol=DSolve[eqns,{x[t],y[t]},{t,0,5}]
(* Output *)
{{x[t]->{, {{t, t<=0}, {(1)/(3) (27 t+3 a t-9 t^2-3 a t^2+t^3+a t^3), 0<t<=1}, {(1)/(6) (-2 a+54 t+9 a t-18 t^2-6 a t^2+2 t^3+a t^3), 1<t<=2}, {(1)/(60) (-932 a-192 a^2+540 t+1610 a t+360 a^2 t-180 t^2-980 a t^2-260 a^2 t^2+20 t^3+250 a t^3+90 a^2 t^3-25 a t^4-15 a^2 t^4+a t^5+a^2 t^5), 2<t<=3}, {(1)/(120) (-1864 a-546 a^2+900 t+3220 a t+855 a^2 t-240 t^2-1960 a t^2-520 a^2 t^2+20 t^3+500 a t^3+150 a^2 t^3-50 a t^4-20 a^2 t^4+2 a t^5+a^2 t^5), 3<t<=4}, {(1)/(2520)(-419328-162792 a-285898 a^2-66560 a^3+395220 t+195300 a t+390691 a^2 t+98560 a^3 t-132720 t^2-93240 a t^2-220584 a^2 t^2-61824 a^3 t^2+20580 t^3+21000 a t^3+65870 a^2 t^3+21280 a^3 t^3-1470 t^4-2100 a t^4-11060 a^2 t^4-4340 a^3 t^4+42 t^5+84 a t^5+1029 a^2 t^5+525 a^3 t^5-49 a^2 t^6-35 a^3 t^6+a^2 t^7+a^3 t^7), 4<t<=5}, {Indeterminate, True}}},y[t]->{, {{t^2, t<=0}, {(1)/(2) (-2 t+t^2), 0<t<=1}, {(1)/(12) (61+11 a-148 t-28 a t+96 t^2+24 a t^2-16 t^3-8 a t^3+t^4+a t^4), 1<t<=2}, {(1)/(24) (122+38 a-296 t-72 a t+192 t^2+48 a t^2-32 t^3-12 a t^3+2 t^4+a t^4), 2<t<=3}, {(1)/(360) (1830+15393 a+3402 a^2-4440 t-22788 a t-5508 a^2 t+2880 t^2+13275 a t^2+3645 a^2 t^2-480 t^3-3780 a t^3-1260 a^2 t^3+30 t^4+540 a t^4+240 a^2 t^4-36 a t^5-24 a^2 t^5+a t^6+a^2 t^6), 3<t<=4}, {(1)/(720) (3660+30786 a+8852 a^2-6960 t-45576 a t-12552 a^2 t+4320 t^2+26550 a t^2+7290 a^2 t^2-600 t^3-7560 a t^3-2200 a^2 t^3+30 t^4+1080 a t^4+360 a^2 t^4-72 a t^5-30 a^2 t^5+2 a t^6+a^2 t^6), 4<t<=5}, {Indeterminate, True}}}}}
```

Plot the solution for different values of the parameter `a`:

```wolfram
ParametricPlot[Evaluate[Table[{x[t],y[t]}/.sol,{a,-1,2,1/3}]],{t,-2,5},Exclusions->None]
(* Output *)
...
```wolfram
DSolve[{y''[x]-y[x]==Max[x,x^2],y[0]==1,y'[0]==2},y[x],x]//Simplify
(* Output *)
{{y[x]->{, {{(1)/(2) (ℯ^-x+5 ℯ^x-2 (2+x^2)), x<=0}, {-ℯ^-x+2 ℯ^x-x, 0<x<=1}, {(1)/(2) (ℯ^(1-x)+3 ℯ^(-1+x)-2 ℯ^-x+4 ℯ^x-2 (2+x^2)), True}}}}}
```

A differential equation with a piecewise coefficient:

```wolfram
DSolve[y'[x]+Clip[x]^2y[x]==0,y[x],x]
(* Output *)
{{y[x]->ℯ^({, {{-x, x<=-1}, {(2)/(3)-(x^3)/(3), -1<x<=1}, {(4)/(3)-x, True}}}) 1}}
```

A nonlinear piecewise-defined differential equation:

```wolfram
sol = DSolve[{y'[x]==Piecewise[{{Cos[x]^2,x >2}}, y[x]^2/3],y[0] == 1},y,x]
(* Output *)
{{y->Function[{x},{, {{-(3)/(-3+x), x<=2}, {(1)/(4) (8+2 x-Sin[4]+Sin[2 x]), True}}}]}}
```
...
```wolfram
DSolve[y''[x]-y[x]==HeavisideTheta[x],y[x],x]
(* Output *)
{{y[x]->ℯ^x 1+ℯ^-x 2+(1)/(2) ℯ^-x (-1+ℯ^x)^2 HeavisideTheta[x]}}
```

A simple impulse response or Green's function:

```wolfram
DSolve[y'[x]+7y[x]==DiracDelta[x],y[x],x]
(* Output *)
{{y[x]->ℯ^(-7 x) 1+ℯ^(-7 x) HeavisideTheta[x]}}
```

Solve a piecewise differential equation on the interval `{0,1}`:

```wolfram
DSolve[y'[x]==UnitStep[x],y,{x,0,1}]
(* Output *)
{{y->Function[{x},x+1]}}
```
...
```wolfram
DSolve[y'[x]==UnitStep[x],y,{x,-1,0}]
(* Output *)
{{y->Function[{x},1]}}
```

```wolfram
DSolve[y'[x]==UnitStep[x],y,{x,-1,1}]
(* Output *)
{{y->Function[{x},1+x UnitStep[x]]}}
```
...
```wolfram
sol=DSolve[{x'[t]==x[t],x[0]==1,WhenEvent[Mod[t,1]==0,x[t]->x[t]+10]},x,{t,0,3}]
(* Output *)
{{x->Function[{t},{, {{ℯ^t, 0<=t<=1}, {ℯ^(-1+t) (10+ℯ), 1<t<=2}, {ℯ^(-2+t) (10+10 ℯ+ℯ^2), 2<t<=3}, {Indeterminate, True}}}]}}
```

Plot the solution:

```wolfram
Plot[x[t]/.sol[[1]],{t,0,4},Exclusions->None]
```
...
(* Output *)
{{y[t]->{, {{1-5 t^2, 0<=t<=(1)/(Sqrt[5])}, {-(12)/(5)+(17 t)/(Sqrt[5])-5 t^2, (1)/(Sqrt[5])<t<=(12)/(5 Sqrt[5])}, {-(1014)/(125)+(289 t)/(10 Sqrt[5])-5 t^2, (12)/(5 Sqrt[5])<t<=(169)/(50 Sqrt[5])}, {-(343577)/(25000)+(3723 t)/(100 Sqrt[5])-5 t^2, (169)/(50 Sqrt[5])<t<=(2033)/(500 Sqrt[5])}, {-(46212123)/(2500000)+(43061 t)/(1000 Sqrt[5])-5 t^2, (2033)/(500 Sqrt[5])<t<=2}, {Indeterminate, True}}},z[t]->{, {{-10 t, 0<=t<=(1)/(Sqrt[5])}, {(17)/(Sqrt[5])-10 t, (1)/(Sqrt[5])<t<=(12)/(5 Sqrt[5])}, {(289)/(10 Sqrt[5])-10 t, (12)/(5 Sqrt[5])<t<=(169)/(50 Sqrt[5])}, {(3723)/(100 Sqrt[5])-10 t, (169)/(50 Sqrt[5])<t<=(2033)/(500 Sqrt[5])}, {(43061)/(1000 Sqrt[5])-10 t, (2033)/(500 Sqrt[5])<t<=2}, {Indeterminate, True}}}}}
```

Plot the solution for `*y*`:

```wolfram
Plot[y[t]/.sol[[1]], {t,0,2}]
```
...
```wolfram
DSolve[{x'[t]==x[t],x[0]==1,WhenEvent[t==2,"StopIntegration"]},x,{t,0,3}]
(* Output *)
{{x->Function[{t},{, {{ℯ^t, 0<=t<=2}, {Indeterminate, True}}}]}}
```

Remove an event after it has occurred once:

```wolfram
sol=DSolve[{x''[t]+x[t]==0,x[0]==0,x'[0]==1,WhenEvent[x[t]==0,{x'[t]->-x'[t],"RemoveEvent"}]},x,{t,0,10}]
(* Output *)
{{x->Function[{t},{, {{Sin[t], 0<=t<=π}, {-Sin[t], π<t<=10}, {Indeterminate, True}}}]}}
```
...
```wolfram
sol=DSolve[{x'[t]==y[t],y'[t]==1,x[0]==0,y[0]==0,WhenEvent[t==1,x[t]->x[t]+1],WhenEvent[t==2,y[t]->y[t]+2]},{x,y},{t,0,3}]
(* Output *)
{{x->Function[{t},{, {{(t^2)/(2), 0<=t<=1}, {(1)/(2) (2+t^2), 1<t<=2}, {(1)/(2) (-6+4 t+t^2), 2<t<=3}, {Indeterminate, True}}}],y->Function[{t},{, {{t, 0<=t<=1||1<t<=2}, {2+t, 2<t<=3}, {Indeterminate, True}}}]}}
```
...
```wolfram
sol=DSolve[{x'[t]==1,x[0]==0,WhenEvent[{t==1,t==2},x[t]->x[t]+1]},x,{t,0,3}]
(* Output *)
{{x->Function[{t},{, {{t, 0<=t<=1}, {1+t, 1<t<=2}, {2+t, 2<t<=3}, {Indeterminate, True}}}]}}
```
...
(* Output *)
{{y[x]->{, {{1 Sin[x Sqrt[λ]], n∈Integers&&n>=1&&λ==n^2}, {0, True}}}}}
```

Make a table of the first 5 eigenfunctions:

```wolfram
eigfuns=Table[y[x]/.sol[[1]]//.{n->i,λ->n^2}/.{1-> 1},{i,5}]
(* Output *)
...
sol=DSolve[{y''[x]+λ y[x]==0,y'[0]==0,y'[π]==0},y[x],x]
(* Output *)
{{y[x]->{, {{1 Cos[x Sqrt[λ]], n∈Integers&&n>=0&&λ==n^2}, {0, True}}}}}
```

Make a table of the first 5 eigenfunctions:

```wolfram
eigfuns=Table[y[x]/.sol[[1]]//.{n->i,λ->n^2}/.{1-> 1},{i,0,4}]
(* Output *)
...
sol=DSolve[{y''[x]+λ y[x]==0,y[0]==0,y'[π]==0},y[x],x]
(* Output *)
{{y[x]->{, {{1 Sin[x Sqrt[λ]], n∈Integers&&n>=1&&λ==(-(1)/(2)+n)^2}, {0, True}}}}}
```

Make a table of the first 5 eigenfunctions:

```wolfram
eigfuns=Table[y[x]/.sol[[1]]//.{n->i,λ->(-(1)/(2)+n)^2}/.{1-> 1},{i,5}]
(* Output *)
...
```wolfram
sol=DSolve[{y''[x]+λ y[x]==0,y[0]+y'[0]==0,y[1]==0},y[x],x]
(* Output *)
{{y[x]->{, {{1 (-Sqrt[λ] Cos[x Sqrt[λ]]+Sin[x Sqrt[λ]]), Sqrt[λ] Cos[Sqrt[λ]]==Sin[Sqrt[λ]]}, {0, True}}}}}
```
...
```wolfram
sol=y[x]/.DSolve[eqns,y[x],x,Assumptions->1<λ<200][[1]];
```
...
(* Output *)
{{y[x]->(6 x)/(λ)-(6 Sin[x Sqrt[λ]])/(λ^(3/2))}}
```

Plot the solution:

```wolfram
Plot[Table[y[x]/.sol[[1]],{λ,1,3,0.5}]//Evaluate,{x,0,20}]
(* Output *)
...
sol=DSolve[eqn,y[x],x]
(* Output *)
{{y[x]->(56 x-8 a x-56 a Sin[7 x]+7 a π^2 Sin[7 x])/(7 a (-8+π^2))}}
```

Plot the solution:

```wolfram
Plot[Table[y[x]/.sol[[1]],{a,-1,4,0.7}]//Evaluate,{x,0,Pi/2}]
(* Output *)
...
(* Output *)
{{y[x]->ℯ^-x (-1+(1+ℯ^(2 x)) 1)-(a Cos[a x])/(1+a^2)}}
```
...
```wolfram
sol2=DSolve[{eqn,init},y[x],x]
(* Output *)
{{y[x]->(ℯ^-x (-2+a-2 a^2+a ℯ^(2 x)-2 a ℯ^x Cos[a x]))/(2 (1+a^2))}}
```

Plot the solution:

```wolfram
Plot[Table[y[x]/.sol2[[1]],{a,-1,4,0.7}]//Evaluate,{x,0,3}]
```
...
sol=DSolve[eqn,y[x],x]
(* Output *)
{{y[x]->Sqrt[a] Sqrt[(2)/(π)] (Cos[a x] FresnelC[Sqrt[(2)/(π)] Sqrt[a x]]+FresnelS[Sqrt[(2)/(π)] Sqrt[a x]] Sin[a x])}}
```

Plot the solution:

```wolfram
Plot[Table[y[x]/.sol[[1]],{a,1,3,0.7}]//Evaluate,{x,0,9}]
```
...
sol=DSolve[eqn,y[x],x]
(* Output *)
{{y[x]->x^a+(ℯ^(π x) π^-a Gamma[1+a] Gamma[(1)/(2)+a,π x])/(Gamma[(1)/(2)+a])-ℯ^(π x) π^-a Gamma[1+a,π x]}}
```

Plot the solution:

```wolfram
Plot[Table[y[x]/.sol[[1]],{a,1,4,0.7}]//Evaluate,{x,0,2}]
```
...
sol=DSolve[eqn,y[x],x]
(* Output *)
{{y[x]->{, {{(2 2 Sin[x])/(π), λ==-(2)/(π)}, {(2 1 Cos[x])/(π), λ==(2)/(π)}, {0, True}}}}}
```

Plot the solution:

```wolfram
Plot[Table[y[x]/.sol[[1]]/.{ 1-> 1, 2->1},{λ,{-2/Pi,2/Pi}}]//Evaluate,{x,0,3π}]
```
...
(* Output *)
{{u->Function[{x,y},ℯ^(-x^2-y^2)]}}
```

```wolfram
Plot3D[u[x,y]/.%[[1]], {x,-2,2}, {y,-2, 2},ColorFunction->"TemperatureMap"]
```
...
```wolfram
DSolve[{D[u[t,x],t]+D[u[t,x],x]==0,u[t,0]==0,u[0,x]==Sin[x]},u,{t,x}]
(* Output *)
{{u->Function[{t,x},-HeavisideTheta[-t+x] Sin[t-x]]}}
```

```wolfram
Plot3D[u[t,x]/.%[[1]]//Evaluate,{t,0,3},{x,0,3},Exclusions->None]
```
...
(* Output *)
{{u->Function[{t,x},ℯ^(-(-c t+x)^2)]}}
```

Plot the traveling wave with speed `*c*==1:

```wolfram
Plot[u[t,x]/.%[[1]]/.{c-> 1,t-> 3}//Evaluate,{x,0,10},PlotRange->All,Ticks->False,Filling-> Axis,Epilog->Arrow[{{4,4/5},{7,4/5}}]]
(* Output *)
...
```wolfram
DSolve[2D[u[x,y],x]+5D[u[x,y],y]==u[x,y]^2+1,u,{x,y}]//Quiet
(* Output *)
{{u->Function[{x,y},Tan[(1)/(2) (x+2 1[(1)/(2) (-5 x+2 y)])]]}}
```
...
```wolfram
DSolve[u[x,y]==x  D[u[x,y],x]+y D[u[x,y],y]+Sin[D[u[x,y],x]+D[u[x,y],y]] ,u,{x,y}]//Quiet
(* Output *)
{{u->Function[{x,y},x 1+y 2+Sin[1+2]]}}
```
...
```wolfram
DSolve[{weqn,ic},u,{x,t}]
(* Output *)
{{u->Function[{x,t},{, {{(1)/(2) (ℯ^(-(-t+x)^2)+ℯ^(-(t+x)^2))+t, t>=0}, {Indeterminate, True}}}]}}
```

The wave propagates along a pair of characteristic directions:

```wolfram
Plot3D[Evaluate[u[x,t]/.%[[1]]],{x,-7,7},{t,0,4},PlotRange->All]
```
...
```wolfram
DSolve[ {weqn,ic},u[x,t],{x,t}]//Simplify//
(* Output *)
{{u(x,t)-> | (1)/(2) (t-x+t+x+(x-t)/(3)+(t+x)/(3)) | t>=0
Indeterminate | True}}
```
...
```wolfram
Plot3D[Evaluate[u[x,t]/.%[[1]]],{x,-7,7},{t,0,4},PlotRange->All,PlotPoints->250,Exclusions->None]
(* Output *)
...
```wolfram
sol=DSolve[ {weqn,ic},u[x,t],{x,t}]
(* Output *)
{{u[x,t]->{, {{(1)/(2) (ℯ^(-(-6-t+x)^2)+ℯ^(-(6-t+x)^2)+ℯ^(-(-6+t+x)^2)+ℯ^(-(6+t+x)^2))+(t)/(2), t>=0}, {Indeterminate, True}}}}}
```

Visualize the solution:

```wolfram
Plot3D[Evaluate[u[x,t]/.sol[[1]]],{x,-30,30},{t,0,20},PlotRange->All,Exclusions->All,PlotPoints->30,WorkingPrecision->20]//Quiet
(* Output *)
...
sol = DSolve[{weqn,ic}, u[x,t], {x, t}]
(* Output *)
{{u[x,t]->{, {{(m t^2)/(2)+(1)/(2) (-ℯ^(-(1)/(6) Abs[-t+x]) Cos[3 (-t+x)]-ℯ^(-(1)/(6) Abs[t+x]) Cos[3 (t+x)]-Sin[t-x]+Sin[t+x]), t>=0}, {Indeterminate, True}}}}}
```

Visualize the solution for different values of `*m*`:

```wolfram
Table[Plot3D[Evaluate[u[x,t]/.sol[[1]]],{x,-7,7},{t,0,4},PlotRange->All,Ticks->False],{m,{-1,0,1}}]
(* Output *)
...
```wolfram
sol=DSolve[{weqn,ic,bc},u,{x,t}];
```

Visualize the solution:

```wolfram
Plot3D[Evaluate[u[x,t]/.sol[[1]]],{x,0,12},{t,0,10},Exclusions->None,PlotRange->All,PlotPoints->120]
(* Output *)
...
```wolfram
Plot3D[Evaluate[u[x,t]/.sol[[1]]],{x,0,20},{t,0,2},Exclusions->None,PlotRange->All]
```
...
```wolfram
(sol=FullSimplify[u[x,y,t]/.DSolve[{weqn,ic, bc},u,{x,y,t}][[1]],K[1]∈Integers &&K[1]>=1 && K[3]∈Integers &&K[3]>=1])//
(* Output *)
...
```wolfram
(sol=u[r,t]/.DSolve[{reqn,ic,bc},u[r,t], {r,t}][[1]]//FullSimplify)//
(* Output *)
...
```wolfram
DSolve[12 D[u[x,t],{x,2}]==D[u[x,t],{t,2}]+D[u[x,t],x,t], u,{x,t}]
(* Output *)
{{u->Function[{x,t},1[t-(x)/(4)]+2[t+(x)/(3)]]}}
```

Hyperbolic partial differential equation with non-rational coefficients:

```wolfram
DSolve[D[u[x,y],{x,2}]-2Sin[x] D[u[x,y],x,y]-Cos[x]^2 D[u[x,y],{y,2}]-Cos[x]D[u[x,y],y]==0, u, {x,y} ]
(* Output *)
{{u->Function[{x,y},1[x+y-Cos[x]]+2[-x+y-Cos[x]]]}}
```

Inhomogeneous hyperbolic partial differential equation with constant coefficients:

```wolfram
DSolve[3D[u[x,t],{x,2}]-D[u[x,t],{t,2}]+D[u[x,t],x,t]==1,u,{x,t}]
(* Output *)
{{u->Function[{x,t},(x^2)/(6)+1[t-(1)/(6) (1+Sqrt[13]) x]+2[t-(1)/(6) (1-Sqrt[13]) x]]}}
```
...
(* Output *)
{{u[x,t]->(1)/(2)+t+(1)/(2) Cos[2 x] Cosh[2 t]+Cos[x] Sinh[t],v[x,t]->-t+Cosh[t] Sin[x] (1+2 Cos[x] Sinh[t])}}
```

```wolfram
Plot3D[({u[x,t],v[x,t]}/.sol[[1]])//Evaluate,{x,0,4},{t,0,3},PlotRange->{-70,120}]
(* Output *)
...
(* Output *)
{{u[x,t]->(ℯ^(-(x^2)/(1+4 t)))/(Sqrt[4+(1)/(t)] Sqrt[t])}}
```
...
```wolfram
Plot[Evaluate[Table[u[x, t] /. sol[[1]], {t, 0, 4}]], {x,-5,5},PlotRange->All,Filling->Axis]//Quiet
```
...
sol=DSolve[{heqn,ic },u[x,t],{x,t}]
(* Output *)
{{u[x,t]->(1)/(2) (Erf[(1-2 x)/(4 Sqrt[t])]+Erf[(1+2 x)/(4 Sqrt[t])])}}
```

Discontinuities in the initial data are smoothed instantly:

```wolfram
Plot3D[Evaluate[u[x,t]/.sol[[1]]],{x,-2,2},{t,0,1},PlotRange->All,PlotPoints->250]
(* Output *)
...
```wolfram
Table[Plot3D[Evaluate[u[x,t]/.sol[[1]]]/.{m->i},{x,-7,7},{t,0,4},PlotRange->All,Axes->False],{i,{2,0,-2}}]
(* Output *)
...
(* Output *)
{{u[x,t]->(108 (2+(-1)^K[1]) ℯ^(-(1)/(9) π^2 t K[1]^2) Sin[(1)/(3) π x K[1]])/(π^3 K[1]^3)}}
```
...
```wolfram
(sol=u[r,t]/.DSolve[{rheqn,ic,bc},u[r,t], {r,t}][[1]])//
(* Output *)
...
```wolfram
DSolve[{BlackScholesPDE,bc},v[t,s],{t,s}]
(* Output *)
{{v[t,s]->(ℯ^(r (t-T)) ∫_-∞^∞ℯ^(-(((-t+T) (-q+r-(σ^2)/(2))-K[1]+Log[s])^2)/(2 (-t+T) σ^2)) ψ[ℯ^K[1]]ⅆK[1])/(Sqrt[2 π] Sqrt[(-t+T) σ^2])}}
```
...
(* Output *)
{{u[x,y]->{, {{({, {{ArcTan[((1)/(2)-x)/(y)]+ArcTan[((1)/(2)+x)/(y)], y>0||x>(1)/(2)||x<-(1)/(2)}, {0, True}}})/(π), y>=0}, {Indeterminate, True}}}}}
```

Discontinuities in the boundary data are smoothed out:

```wolfram
Plot3D[u[x,y]/.sol[[1]],{x,-3,3},{y,0,5},PlotRange->All]
```
...
sol=DSolve[{leqn,bc},u[x,y], {x,y}]
(* Output *)
{{u[x,y]->{, {{(x+(x Cos[y]-y Sin[y]) (-Cosh[x]+Sinh[x]))/(x^2+y^2), x>=0}, {Indeterminate, True}}}}}
```

Visualize the solution:

```wolfram
Plot3D[u[x,y]/.sol[[1]],{y,-10,10},{x,0,5},PlotRange->All]
```
...
```wolfram
Plot3D[u[x,y]/.sol[[1]],{x,0,10},{y,0,7},PlotRange->All]
```
...
sol=DSolve[{leqn,bc},u[x,y], {x,y}]
(* Output *)
{{u[x,y]->{, {{(1)/(4 π)(-4+4 y ArcCot[(2 y)/(1-2 x)]+4 y ArcTan[((1)/(2)+x)/(y)]-2 Log[4]+Log[(1-2 x)^2+4 y^2]-2 x Log[(1-2 x)^2+4 y^2]+Log[(1+2 x)^2+4 y^2]+2 x Log[(1+2 x)^2+4 y^2]), y>=0}, {Indeterminate, True}}}}}
```
...
```wolfram
sol=DSolve[{leqn,bc},u[r,θ],{r,θ}]
(* Output *)
{{u[r,θ]->(1)/(729) r^6 Sin[6 θ]}}
```

Visualize the solution:

```wolfram
Plot3D[Evaluate[u[r,θ]/.sol[[1]]/.{r->Sqrt[x^2+y^2], θ->ArcTan[y/x]}],{x,y}∈Disk[{0,0},3],PlotRange->All,Exclusions->None]
```
...
sol=DSolve[{leqn,bc},u[r,θ],{r,θ}]
(* Output *)
{{u[r,θ]->{, {{(5 Log[r])/(Log[2]), 1<=r<=2}, {Indeterminate, True}}}}}
```

Visualize the solution:

```wolfram
Plot3D[Evaluate[u[r,θ]/.sol[[1]]/.{r->Sqrt[x^2+y^2], θ->ArcTan[y/x]}],{x,y}∈Annulus[{0,0},{1,2}],PlotRange->All,Exclusions->None]
```
...
```wolfram
sol=DSolve[{peqn,bc},u[x,y],{x,y}]
(* Output *)
{{u[x,y]->1+11 x+x^3-y^3}}
```

Dirichlet problem for the Helmholtz equation in a rectangle:

```wolfram
heqn={Laplacian[u[x,y],{x,y}]+5u[x,y]==0};
```
...
```wolfram
(sol=u[x,y]/.DSolve[{heqn,bc},u[x,y],{x,y}][[1]])//
(* Output *)
...
(* Output *)
{{ψ[x,t]->ℯ^(-(ⅈ π^2 t ℏ K[1]^2)/(2 (-a+b)^2 m)) K[1] Sin[(π (-a+x) K[1])/(-a+b)]}}
```

Extract the first four terms in the solution:

```wolfram
u[x_,t_]=TruncateSum[ψ[x,t] /.First[%],4]
(* Output *)
...
(* Output *)
{{ψ->Function[{x,t},(100 (7+8 (-1)^K[1]) ℯ^(-(2)/(25) ⅈ π^2 (-2+t) K[1]^2) Sin[(1)/(5) π (-5+x) K[1]])/(π^3 K[1]^3)]}}
```

Define a family of partial sums of the solution:

```wolfram
u[k_Integer]=ψ /.TruncateSum[First[sol],k]
(* Output *)
...
```wolfram
sol=DSolve[{eqn,ψ[-∞,t]==0,ψ[∞,t]==0},ψ[x,t],{x,t}]
(* Output *)
{{ψ[x,t]->ℯ^(-(x^2)/(Sqrt[2])-2 ⅈ Sqrt[2] t ((1)/(2)+K[1])) K[1] HermiteH[K[1],2^(1/4) x]}}
```

Extract the first two terms in the solution:

```wolfram
u[x_,t_]=TruncateSum[ψ[x,t] /.First[sol],2]
(* Output *)
...
```wolfram
sol=DSolve[{BurgersEqn,u[x,0]==UnitBox[x]}, u[x,t],{x,t}];(dsol=u[x,t]/.sol[[1]]//FullSimplify)//
(* Output *)
...
```wolfram
sol = DSolve[KdV,u[x,t],{x,t}]//Quiet
(* Output *)
{{u[x,t]->-(-8 1^3+2+12 1^3 Tanh[x 1+t 2+3]^2)/(6 1)}}
```

Obtain a particular solution for a fixed choice of arbitrary constants:

```wolfram
psol=u[x,t]/.sol[[1]]/.{1->1,2->-4,3->1/4}//Simplify
(* Output *)
...
```wolfram
(sol=FullSimplify[DSolve[{leqn,dcond},u[x,y],{x,y}∈Ω]])//
(* Output *)
{{u(x,y)->(8 cosh(π (y-1) K[1]) sech(π K[1]) sin((1)/(2) π K[1]) sin(π x K[1]))/(π^(2) K[1]^(2))}}
```

Extract the first `100 terms from the [Inactive](https://reference.wolfram.com/language/ref/Inactive.html) sum:

```wolfram
asol=TruncateSum[u[x,y]/.sol[[1]],100];
```
...
```wolfram
sol=DSolve[{leqn,dcond},u[x,y],{x,y}∈Ω]
(* Output *)
...
```wolfram
Plot3D[u[x,y]/.sol[[1]]//Evaluate,{x,y}∈Ω,PlotRange->All,PlotStyle->Hue[0.5]]
```
...
```wolfram
Plot3D[u[x,y]/.sol[[1]]//Evaluate,{x,y}∈Ω,PlotRange->All,PlotStyle->Green]
```
...
```wolfram
sol=DSolve[CaputoD[y[x],{x,1/2}]-3 y[x]==0,y[x],x]
(* Output *)
{{y[x]->ℯ^(9 x) 1 Erfc[-3 Sqrt[x]]}}
```

Verify the solution:

```wolfram
CaputoD[y[x],{x,1/2}]==3y[x]/.sol//FullSimplify
(* Output *)
{True}
```
...
```wolfram
sol=DSolve[{CaputoD[y[x],{x,1/2}]-3 y[x]==0,y[0]==5},y[x],x]
(* Output *)
{{y[x]->5 ℯ^(9 x) Erfc[-3 Sqrt[x]]}}
```
...
```wolfram
sol=DSolve[CaputoD[y[x],{x,0.7}]+3 y[x]==0,y[x],x]
(* Output *)
{{y[x]->1. 1 MittagLefflerE[0.7,-3. x^0.7]}}
```

Verify the solution:

```wolfram
CaputoD[y[x],{x,0.7}]+3y[x]==0/.sol//FullSimplify
(* Output *)
{True}
```
...
```wolfram
sol=DSolve[{CaputoD[y[x],{x,1/7}]==8 (1-y[x]),y[0]==2},y,x]
(* Output *)
{{y->Function[{x},1+MittagLefflerE[(1)/(7),-8 x^(1/7)]]}}
```

Verify the solution:

```wolfram
{CaputoD[y[x],{x,1/7}]==8 (1-y[x]),y[0]==2}/.sol//Simplify
(* Output *)
{{True,True}}
```
...
```wolfram
sol=DSolve[eqns,{x1,x2},t]
(* Output *)
{{x1->Function[{t},1. MittagLefflerE[0.95,-2 t^0.95]+0.2 MittagLefflerE[0.95,t^0.95]],x2->Function[{t},4. MittagLefflerE[0.95,-2 t^0.95]+0.2 MittagLefflerE[0.95,t^0.95]]}}
```
...
```wolfram
ParametricPlot[Evaluate[{x1[t],x2[t]}/.sol],{t,0,2}]
```
...
(* Output *)
{{x[t]->{(-(3)/(2)+(5 ⅈ)/(2)) MittagLefflerE[(16)/(17),-ⅈ t^(16/17)]-((3)/(2)+(5 ⅈ)/(2)) MittagLefflerE[(16)/(17),ⅈ t^(16/17)],((5)/(2)+(3 ⅈ)/(2)) MittagLefflerE[(16)/(17),-ⅈ t^(16/17)]+((5)/(2)-(3 ⅈ)/(2)) MittagLefflerE[(16)/(17),ⅈ t^(16/17)]}}}
```
...
```wolfram
ParametricPlot[Evaluate[x[t]/.sol[[1]]],{t,0,20}]
```
...
```wolfram
sol=DSolve[{fweqn,ic},y,{t,x}]
(* Output *)
{{y->Function[{t,x},MittagLefflerE[(19)/(10),-π^2 t^(19/10)] Sin[π x]]}}
```

Plot the solution:

```wolfram
Plot3D[Evaluate[y[t,x]/.sol],{x,-5,5},{t,0,1},PlotPoints->25]
(* Output *)
...
```wolfram
DSolve[y''[x]+4y[x]==0,y,x]
(* Output *)
{{y->Function[{x},1 Cos[2 x]+2 Sin[2 x]]}}
```

One boundary condition:

```wolfram
DSolve[{y''[x]+4y[x]==0,y[0]==1},y,x]
(* Output *)
{{y->Function[{x},Cos[2 x]+2 Sin[2 x]]}}
```

Two boundary conditions:

```wolfram
DSolve[{y''[x]+4y[x]==0,y[0]==1,y'[0]==4},y,x]
(* Output *)
{{y->Function[{x},Cos[2 x]+2 Sin[2 x]]}}
```
...
```wolfram
DSolve[{y''[x]+λy[x]==0,y[0]==0,y[π]==0},y[x],x]
(* Output *)
{{y[x]->{, {{1 Sin[x Sqrt[λ]], n∈Integers&&n>=1&&λ==n^2}, {0, True}}}}}
```
...
```wolfram
DSolve[{y''[x]+λy[x]==0,y[0]==0,y[π]==0},y[x],x,Assumptions->0<λ<20]
(* Output *)
{{y[x]->{, {{1 Sin[x Sqrt[λ]], λ==1||λ==4||λ==9||λ==16}, {0, True}}}}}
```

Specify that the independent variable `*x*` is real:

```wolfram
eqn={y'[x]==Abs[x+Abs[x]] y[x],y[0]==1};
```

```wolfram
sol=DSolve[eqn,y,x,Assumptions->x∈Reals]
(* Output *)
{{y->Function[{x},{, {{1, x<=0}, {ℯ^(x^2), True}}}]}}
```
...
```wolfram
sol=DSolve[{x'[t] ==a[t],x[0]==0, a[0]==2,WhenEvent[Mod[x[t],1]==0,a[t]->- a[t]]},{x,a},{t,0,2},DiscreteVariables->a]
(* Output *)
{{x->Function[{t},{, {{2 t, 0<=t<=(1)/(2)}, {2-2 t, (1)/(2)<t<=1}, {2 (-1+t), 1<t<=(3)/(2)}, {4-2 t, (3)/(2)<t<=2}, {Indeterminate, True}}}],a->Function[{t},{, {{2, 0<=t<=(1)/(2)}, {-2, (1)/(2)<t<=1}, {2, 1<t<=(3)/(2)}, {-2, (3)/(2)<t<=2}, {Indeterminate, True}}}]}}
```
...
```wolfram
DSolve[y''[x]==y[x],y[x],x,GeneratedParameters->(c_#&)]
(* Output *)
{{y[x]->ℯ^x c_1+ℯ^-x c_2}}
```
...
```wolfram
DSolve[y''[x]==y[x],y[x],x, GeneratedParameters->Unique[C]]
(* Output *)
{{y[x]->ℯ^x C$751514[1]+ℯ^-x C$751514[2]}}
```

The constants of integration are unique across different invocations of [DSolve](https://reference.wolfram.com/language/ref/DSolve.html):

```wolfram
DSolve[y''[x]==y[x],y[x],x, GeneratedParameters->Unique[C]]
(* Output *)
{{y[x]->ℯ^x C$751520[1]+ℯ^-x C$751520[2]}}
```
...
```wolfram
DSolve[{y''[x]+y[x]==0,y[0]==0,y'[0]==1},y[x],x,Method->"Holonomic"]
(* Output *)
{{y[x]-><|head -> DifferentialRoot, big -> {{y[x]+y^′′[x]==0}, {y[0]==0}, {y^′[0]==1}}, small -> y^′′[x], branchcuts -> None|>[x]}}
```
...
Use [IncludeSingularSolutions](https://reference.wolfram.com/language/ref/IncludeSingularSolutions.html) to compute singular solutions along with the general solution:

```wolfram
DSolve[y'[x]^2-y'[x]*x+y[x]==0,y[x],x,IncludeSingularSolutions->True]
(* Output *)
{{y[x]->x 1-1^2},{y[x]->(x^2)/(4)}}
```
...
(* Output *)
{{y->Function[{x},(27 a ℯ^x)/(27-a+a ℯ^x)]}}
```
...
```wolfram
DSolve[{y''[x]+3y'[x]+40 y[x]==0,y[0]==1,y'[0]==1/3},y,x]
(* Output *)
{{y->Function[{x},(1)/(453) ℯ^(-3 x/2) (453 Cos[(Sqrt[151] x)/(2)]+11 Sqrt[151] Sin[(Sqrt[151] x)/(2)])]}}
```
...
```wolfram
DSolve[{x'[t]==-2y[t]+3x[t],y'[t]==15x[t]-y[t],x[0]==a,y[0]==b},{x,y}, t];
```

```wolfram
ParametricPlot[Evaluate[Table[{x[t], y[t]}/.%/.{a->1/(13+m), b->1/(15+m)}, {m,0,20,7}]], {t,-2,2}]
(* Output *)
...
(* Output *)
{{y->Function[{x},ℯ^(-ℯ^x) (3 ℯ-ExpIntegralEi[1]+ExpIntegralEi[ℯ^x])]}}
```
...
```wolfram
logisticeqn=y'[x]==r y[x](1-y[x]);
```

```wolfram
DSolve[{logisticeqn,y[0]==0},y[x],x]
(* Output *)
{{y[x]->0}}
```

```wolfram
DSolve[{logisticeqn,y[0]==1},y[x],x]
(* Output *)
{{y[x]->1}}
```
...
```wolfram
{iL1[r_,c_,l_,t_],vC1[r_,c_,l_,t_]}={iL[t],vC[t]}/.sol[[1]];
```

Visualize the response for different values of $R$, $L$ and $C$:

```wolfram
Manipulate[{Plot[Evaluate[{v1[t],Re[vC1[r,c,l,t]]}],{t,0,.03},Filling->{1->0},Ticks->{Range[0.01,0.03,0.01],Automatic},PlotRange->All,PlotLabel->"V_C[t]"],Plot[Evaluate[Re[iL1[r,c,l,t]]],{t,0,.03},Ticks->{Range[0.01,0.03,0.01],Automatic},PlotRange->All,PlotLabel->"I_L[t]"]},{{r,1,"R"},1,10,1},{{l,10^-2,"L"},10^-2,5 10^-2,10^-2},{{c,10^-4,"C"},10^-4,5 10^-4,10^-4},SaveDefinitions->True]
```
...
```wolfram
sol=DSolve[Flatten[{massConservation,ic}],{h1,h2},t]
(* Output *)
{{h1->Function[{t},(a1+a2 ℯ^(((-a1-a2) g π pipeDia^4 t ρ)/(128 a1 a2 pipeLen μ)))/(a1+a2)],h2->Function[{t},-(a1 (-1+ℯ^(((-a1-a2) g π pipeDia^4 t ρ)/(128 a1 a2 pipeLen μ))))/(a1+a2)]}}
```

```wolfram
{h1[t_],h2[t_]}={h1[t],h2[t]}/.sol[[1]];
```
...
```wolfram
DSolve[{CaputoD[x[t],{t,1.9}]+ x[t]==0,x[0]==1,x'[0]==0},x[t],t]
(* Output *)
{{x[t]->1. MittagLefflerE[1.9,-1. t^1.9]}}
```

Plot the solution:

```wolfram
fracsol=Plot[x[t]/.%,{t,0,20}]
```
...
```wolfram
dampedOsc=DSolve[{x''[t]+a x'[t]+ x[t]==0,x[0]==1,x'[0]==0},x[t],t]//FullSimplify
(* Output *)
{{x[t]->ℯ^(-(a t)/(2)) (Cosh[(1)/(2) Sqrt[-4+a^2] t]+(a Sinh[(1)/(2) Sqrt[-4+a^2] t])/(Sqrt[-4+a^2]))}}
```
...
```wolfram
LCsol[α_,l_,c_]:=DSolve[LCeqn[α,l,c],i,t];
```
...
LCsol[2,l,c]
(* Output *)
{{i->Function[{t},0.1 Cos[(t)/(Sqrt[c] Sqrt[l])]]}}
```
...
LCsol[1.9,l,c]
(* Output *)
{{i->Function[{t},0.1 MittagLefflerE[1.9,-(t^1.9)/(c l)]]}}
```

Plot the solutions for various values of equation order $\alpha$:

```wolfram
Plot[Evaluate[i[t]/.{LCsol[2,1,1],LCsol[1.9,1,1],LCsol[1.8,1,1]}],{t,0,10},PlotLegends->{"α=2","α=1.9","α=1.8"}]
```
...
```wolfram
RCsol[α_,r_,c_]:=DSolve[RCeqn[α,r,c],v,t]
```
...
RCsol[0.9,r,c]
(* Output *)
{{v->Function[{t},20. MittagLefflerE[0.9,-(t^0.9)/(c r)]]}}
```

Plot the solutions for various values of equation order $\alpha$:

```wolfram
Plot[Evaluate[v[t]/.{RCsol[1,10,1],RCsol[0.9,10,1],RCsol[0.8,10,1]}],{t,0,100},PlotLegends->{"α=1","α=0.9","α=0.8"}]
```
...
```wolfram
sol=DSolve[{x''[t]+x[t]==0,x[0]==0,x'[0]==1,WhenEvent[x[t]==0,x'[t]->-x'[t]]},x,{t,0,4π}]
(* Output *)
{{x->Function[{t},{, {{Sin[t], 0<=t<=π}, {-Sin[t], π<t<=2 π}, {Sin[t], 2 π<t<=3 π}, {-Sin[t], 3 π<t<=4 π}, {Indeterminate, True}}}]}}
```

```wolfram
Plot[{Sin[t],(x[t]/.sol)},{t,0,4π}]
```
...
```wolfram
sol=DSolve[{x'[t]==a[t],y'[t]==b[t],x[0]==0,y[0]==0,a[0]==1,b[0]==17/12,WhenEvent[x[t]^2==1,a[t]->-a[t]],WhenEvent[y[t]^2==1,b[t]->-b[t]]},{x,y},{t,0,50}, DiscreteVariables->{a,b}];
```

```wolfram
ParametricPlot[{x[t],y[t]}/.sol,{t,0,50},Frame->True,FrameTicks->None,PlotRange->1,Axes->False]
```
...
```wolfram
DSolve[{system,control},{y,u},{t,0,20},DiscreteVariables->u];
```

```wolfram
Plot[Evaluate[{u[t],y[t]}/.%], {t, 0, 10}, PlotRange->{-6,6},Exclusions->None]
```
...
```wolfram
DSolve[{system,control},{x,u},{t,0,2},DiscreteVariables->u];
```

```wolfram
Plot[Evaluate[{x[t],x'[t],u[t]}/.%], {t,0,2}, PlotRange->All,Exclusions->None]
```
...
```wolfram
DSolve[{system,control},{x,u},{t, 0,12}, DiscreteVariables->u];
```

```wolfram
Plot[Evaluate[{xref,x[t],u[t]}/.%],{t,0,12},Exclusions->None]
```
...
(* Output *)
{{a->Function[{t},{, {{1-(t)/(11), 0<=t<=11}, {0, 11<t<=15}, {Indeterminate, True}}}],u->Function[{t},{, {{1, 0<=t<=11}, {0, 11<t<=15}, {Indeterminate, True}}}]}}
```

```wolfram
Plot[Evaluate[a[t]/.sol],{t,0,15}, PlotRange -> All]
```
...
```wolfram
ParametricPlot[Evaluate[{x[t-2],x[t]}/.sol], {t,2,30}]
```
...
```wolfram
f[λ_,μ_]:=x[t]/.DSolve[{x'[t]==λ x[t] +μ x[t-1],x[t/;t<=0]==1-t},x[t],{t,0,10}][[1]]
```
...
Use [DSolve](https://reference.wolfram.com/language/ref/DSolve.html) to solve the integral equation:

```wolfram
DSolve[abeleqn,h[y],y]
(* Output *)
{{h[y]->(Sqrt[2] g T)/(π Sqrt[g y])}}
```

Using the relationship $\mathrm{d}s=\sqrt{\mathrm{d}x^{2}+\mathrm{d}y^{2}}$, solve for $x^{'}(y)$:

```wolfram
dxdy = Sqrt[h[y]^2-1] /. First[%]
(* Output *)
...
(* Output *)
(π Sqrt[y (2 g T^2-π^2 y)]+2 g T^2 ArcTan[(π)/(Sqrt[-π^2+(2 g T^2)/(y)])])/(π^2)
```
...
Solve the integro-differential equation using [DSolve](https://reference.wolfram.com/language/ref/DSolve.html):

```wolfram
sol=DSolve[{ieqn,ic},v[t],t]
(* Output *)
{{v[t]->(1)/(14) (7 Cos[2 t]+385 ℯ^(-t/3) Cos[(Sqrt[119] t)/(3)]-49 Sin[2 t]+5 Sqrt[119] ℯ^(-t/3) Sin[(Sqrt[119] t)/(3)])}}
```

Plot the velocity as a function of time:

```wolfram
Plot[v[t]/.sol[[1]],{t,0,8},PlotRange->All, Filling->Axis]
(* Output *)
...
An LRC circuit has a voltage source given by $v(t)=V$. The resistance in the circuit is 0.2 $\Omega$, the inductance is $H$, and the capacitance is $F$. Initially, the current in the resistor is $A$. Find the current as a function of time:
...
```wolfram
sol=DSolve[{ieqn,ic},i[t],t]//FullSimplify//Chop
(* Output *)
{{i[t]->0.03126196248564503 Cos[1. t]+ℯ^(-0.020000000000000004 t) (0.9687380375143549 Cos[0.534148187036412 t]-0.33487817368239675 Sin[0.534148187036412 t])+0.5582493301008038 Sin[1. t]}}
```

Plot the current as a function of time:

```wolfram
Plot[i[t]/.sol[[1]],{t,0,80},PlotRange->All, Filling->Axis]
(* Output *)
...
Solve the integral equation using [DSolve](https://reference.wolfram.com/language/ref/DSolve.html):
...
```wolfram
DSolve[{deqn,init},y[x],x]//Simplify
(* Output *)
{{y[x]->(6 x)/(λ)-(6 Sin[x Sqrt[λ]])/(λ^(3/2))}}
```
...
```wolfram
Plot[Table[y[x]/.%[[1]],{λ,1,3,0.5}]//Evaluate,{x,0,20},Filling->Axis]
(* Output *)
...
```wolfram
dsol=DSolve[{eqn,bc,ic},u,{x,t},Assumptions->Element[m,Integers]]
(* Output *)
{{u->Function[{x,t},Cos[m t] Sin[m x]]}}
```

Visualize the vibrations of the string for these modes:

```wolfram
sol1[x_,t_]=u[x,t]/.dsol[[1]]
(* Output *)
Cos[m t] Sin[m x]
```

```wolfram
Table[Show[Plot[Table[sol1[x,t],{t,0,4}]//Evaluate,{x,0,Pi},Ticks->False]],{m,4}]
(* Output *)
{[Graphics],[Graphics],[Graphics],[Graphics]}
```
...
```wolfram
dsol=DSolve[{eqn,bc,ic},u,{x,t}]/.{K[1]-> m}
(* Output *)
{{u->Function[{x,t},(4 (2+(-1)^m) Cos[t m] Sin[x m])/(m^3)]}}
```

Extract four terms from the [Inactive](https://reference.wolfram.com/language/ref/Inactive.html) sum:

```wolfram
sol[x_,t_]=TruncateSum[u[x,t]/.dsol[[1]],4]
(* Output *)
...
```wolfram
(dsol=DSolve[{eqn,bc,ic},u[r,t],{r,t}])//
(* Output *)
{{u(r,t)->(2 0 1 sin(t 0))/((0^(2)+1^(2)) (0)^(2))}}
```

Extract a finite number of terms from the [Inactive](https://reference.wolfram.com/language/ref/Inactive.html) sum:

```wolfram
h[r_,t_]=TruncateSum[u[r,t]/.dsol[[1]],3]//N;
```
...
(* Output *)
{{u[x,t]->20+30 x-(2 ((20-50 (-1)^K[1]) ℯ^(-π^2 t K[1]^2) Sin[π x K[1]])/(K[1]))/(π)}}
```

Extract a few terms from the [Inactive](https://reference.wolfram.com/language/ref/Inactive.html) sum:

```wolfram
approxsol=TruncateSum[u[x,t]/.sol[[1]],3]//Expand
(* Output *)
...
```wolfram
ssol=DSolve[{v''[x]==0,v[0]==20,v[1]==50},v[x],x]//Expand
(* Output *)
{{v[x]->20+30 x}}
```
...
sol=DSolve[{heqn,bc,ic},u[x,t],{x,t}]
(* Output *)
{{u[x,t]->60+2 (80 (-1+(-1)^K[1]) ℯ^(-π^2 t K[1]^2) Cos[π x K[1]])/(π^2 K[1]^2)}}
```

Extract a few terms from the [Inactive](https://reference.wolfram.com/language/ref/Inactive.html) sum:

```wolfram
approxsol=TruncateSum[u[x,t]/.sol[[1]],4]//Expand
(* Output *)
...
(* Output *)
{{u->Function[{x,y},x^3-3 x y^2],v->Function[{x,y},3 x^2 y-y^3]}}
```
...
```wolfram
ContourPlot[{u[x,y],v[x,y]}/.sol[[1]],{x,-5,5},{y,-5,5},ContourStyle->{Red,Blue},PlotTheme->"Marketing"]
```
...
```wolfram
dsol=DSolve[eqns, u,{x,t}]
(* Output *)
{{u->Function[{x,t},(1)/(1+(ℯ^(-(t-2 x)/(4 ε)) (1+Erf[(x)/(2 Sqrt[t ε])]))/(1+Erf[(t-x)/(2 Sqrt[t ε])]))]}}
```

The solution is smooth for any positive value of `ε`:

```wolfram
Plot3D[u[x,t]/.dsol[[1]]/.{ε-> 1/10},{x,-2,2},{t,0.001,5}]
```
...
```wolfram
Table[Plot3D[u[x,t]/.dsol[[1]]/.{ε-> 1/1000},{x,-2,2},{t,0.001,5},Exclusions->All,Ticks->None],{ε,{1/10,1/100,1/1000}}]//Quiet
(* Output *)
{[Graphics3D],[Graphics3D],[Graphics3D]}
```
...
```wolfram
DSolve[Join[{eqn},bcs],ψ[x,t],{x,t}]
(* Output *)
{{ψ[x,t]->ℯ^(-(ⅈ π^2 t ℏ K[1]^2)/(2 d^2 m)) K[1] Sin[(π x K[1])/(d)]}}
```
...
```wolfram
sol1=DSolve[Join[{eqn,ψ[x,0]==Sqrt[(2)/(d)]Sin[Pi(x)/(d)]},bcs],ψ[x,t],{x,t}]
(* Output *)
{{ψ[x,t]->Sqrt[2] Sqrt[(1)/(d)] ℯ^(-(ⅈ π^2 t ℏ)/(2 d^2 m)) Sin[(π x)/(d)]}}
```

The resulting probability distribution is independent of time:

```wolfram
ρ1[x_,t_]=Simplify[ComplexExpand[Conjugate[ψ[x,t]]ψ[x,t] /. First[sol1]],d>0]
(* Output *)
(2 Sin[(π x)/(d)]^2)/(d)
```
...
```wolfram
sol2=DSolve[Join[{eqn,ψ[x,0]==(1)/(Sqrt[d])Sin[Pi(x)/(d)]+(1)/(Sqrt[d])Sin[2Pi(x)/(d)]},bcs],ψ[x,t],{x,t}]
(* Output *)
{{ψ[x,t]->(ℯ^(-(2 ⅈ π^2 t ℏ)/(d^2 m)) (ℯ^((3 ⅈ π^2 t ℏ)/(2 d^2 m))+2 Cos[(π x)/(d)]) Sin[(π x)/(d)])/(Sqrt[d])}}
```

This density is not stationary, as `*t*` appears in the second and third cosines:

```wolfram
ρ2[x_,t_]=Simplify[ComplexExpand[Conjugate[ψ[x,t]]ψ[x,t] /. First[sol2]],d>0]
(* Output *)
...
```wolfram
dsol=c[t,s]/.DSolve[BlackScholesModel,c[t,s],{t,s}][[1]]
(* Output *)
...
sol=DSolve[{D[f[x,y], x]==x y Cos[x y]+Sin[x y],
                D[f[x,y],y]==-ℯ^-y+x^2 Cos[x y]},f,{x,y}]
(* Output *)
{{f->Function[{x,y},ℯ^-y+1+x Sin[x y]]}}
```

The solution represents a family of parallel surfaces:

```wolfram
Plot3D[Table[f[x,y,z]/.sol[[1]]/.{1->k},{k,1,22,5}]//Evaluate,{x,0,10},{y,0,2},PlotRange->All]
(* Output *)
...
```wolfram
sol=u[x,y]/.DSolve[{(1-x)D[u[x,y],x]==y u[x, y],u[0, y]==1},u, {x,y}][[1]]
(* Output *)
(-1)^y (-1+x)^-y
```
...
(* Output *)
{{y->Function[{x},(1)/(2) ℯ^-x (-3+5 ℯ^(2 x))]}}
```
...
(* Output *)
{{y->Function[{x},1+(1)/(2) Sqrt[π] Erfi[x]]}}
```
...
Use [NDSolve](https://reference.wolfram.com/language/ref/NDSolve.html) to find a numerical solution:
...
Compute an impulse response using [DSolve](https://reference.wolfram.com/language/ref/DSolve.html):

```wolfram
DSolve[y'''[x]-5y''[x]+9y'[x]-5y[x]==DiracDelta''[x]+2DiracDelta'[x]+DiracDelta[x]&&y[-1]==0&&y'[-1]==0&&y''[-1]==0,y[x],x]//FullSimplify
(* Output *)
{{y[x]->-ℯ^x HeavisideTheta[x] (-2+ℯ^x (Cos[x]-7 Sin[x]))}}
```
...
```wolfram
DSolve[{y''[x]+x^2 y'[x]+Tan[y[x]]==1,y[0]==1,y'[0]==1},y,{x,0,1}]
(* Output *)
DSolve[{Tan[y[x]]+x^2 y^′[x]+y^′′[x]==1,y[0]==1,y^′[0]==1},y,{x,0,1}]
```
...
```wolfram
peqn=Laplacian[u[x,y],{x,y}]==0;
```

```wolfram
bc={u[x,0]==0,u[x,1]==0,u[0,y]==y^2-y,u[1,y]==0};
```

```wolfram
u[x,y]/.DSolve[{peqn,bc},u[x,y],{x,y}][[1]]
(* Output *)
(4 (-1+(-1)^K[1]) Csch[π K[1]] Sin[π y K[1]] Sinh[π (1-x) K[1]])/(π^3 K[1]^3)
```
...
```wolfram
rbc=DirichletCondition[u[x,y]==Piecewise[{{y^2-y,x==0}}],True];
```

```wolfram
u[x,y]/.DSolve[{peqn,rbc},u,{x,y}∈Ω][[1]]
(* Output *)
(4 (-1+(-1)^K[1]) Csch[π K[1]] Sin[π y K[1]] Sinh[π (1-x) K[1]])/(π^3 K[1]^3)
```
...
```wolfram
sol=DSolve[{x'[t]==-x[t]-y[t]-1,y'[t]==2x[t]-y[t]+5,x[0]==-2,y[0]==1},{x[t],y[t]},t]//Simplify
(* Output *)
{{x[t]->-2,y[t]->1}}
```
...
```wolfram
sol=DSolve[{x'[t]==-x[t]-y[t]-1,y'[t]==2x[t]-y[t]+5,x[0]==3,y[0]==-2},{x[t],y[t]},t]//Simplify
(* Output *)
{{x[t]->-2+5 ℯ^-t Cos[Sqrt[2] t]+(3 ℯ^-t Sin[Sqrt[2] t])/(Sqrt[2]),y[t]->ℯ^-t (ℯ^t-3 Cos[Sqrt[2] t]+5 Sqrt[2] Sin[Sqrt[2] t])}}
```
...
DSolve
(* Output *)
{{y->Function[{x},-JacobiSN[x,(1)/(2)]]},{y->Function[{x},JacobiSN[x,(1)/(2)]]}}
```
...
```wolfram
DSolve[{y'[x]==0,y[0]==1},y,x]
(* Output *)
DSolve
(* Output *)
DSolve[{-Sin[x]==0,True},y,x]
```
...
Here [DSolve](https://reference.wolfram.com/language/ref/DSolve.html) gives an error message because $z(x)$ is not included in the second argument:

```wolfram
DSolve[{y'[x]==3y[x],z'[x]==2 z[x],z[0]==1,y[0]==1},y[x],x]
(* Output *)
DSolve
(* Output *)
DSolve[{y^′[x]==3 y[x],z^′[x]==2 z[x],z[0]==1,y[0]==1},y[x],x]
```

Include $z(x)$ in the second argument to obtain a solution:

```wolfram
DSolve[{y'[x]==3y[x],z'[x]==2 z[x],z[0]==1,y[0]==1},{y[x],z[x]},x]
(* Output *)
{{y[x]->ℯ^(3 x),z[x]->ℯ^(2 x)}}
```

Alternatively, use [DSolveValue](https://reference.wolfram.com/language/ref/DSolveValue.html) to obtain the solution for $y(x)$ alone:

```wolfram
DSolveValue[{y'[x]==3y[x],z'[x]==2 z[x],z[0]==1,y[0]==1},y[x],x]
(* Output *)
ℯ^(3 x)
```

This example returns unevaluated because [DSolve](https://reference.wolfram.com/language/ref/DSolve.html) is unable to interpret it as a vector equation:
...
eqn=x'[t]==A.x[t]+B
(* Output *)
x^′[t]=={3 t+{{2,0},{-2,0}}.x[t],4+{{2,0},{-2,0}}.x[t]}
```

```wolfram
DSolve[eqn,x∈Vectors[2],t]
(* Output *)
DSolve[x^′[t]=={3 t+{{2,0},{-2,0}}.x[t],4+{{2,0},{-2,0}}.x[t]},x∈Vectors[2,Complexes],t]
```
...
eqn1=x'[t]+A.x[t]==B
(* Output *)
{{2,0},{-2,0}}.x[t]+x^′[t]=={3 t,4}
```

```wolfram
DSolve[eqn1,x∈Vectors[2],t]
(* Output *)
{{x->Function[{t},{3 (-(1)/(4)+(t)/(2))+ℯ^(-2 t) 1,-3 ℯ^(2 t) (-(1)/(4)+(t)/(2))+3 (-1+ℯ^(2 t)) (-(1)/(4)+(t)/(2))+4 t+(3 t^2)/(2)+ℯ^(-2 t) (-1+ℯ^(2 t)) 1+2}]}}
```
...
[DSolve](https://reference.wolfram.com/language/ref/DSolve.html) returns the same solution with a warning message:
...
[DSolve](https://reference.wolfram.com/language/ref/DSolve.html) returns the general solution for this PDE:
...
sol=DSolve[eqn,y[x],x]
(* Output *)
{{y[x]->ℯ^(-(k x)/(2)-(1)/(2) Sqrt[k^2-4 m] x) 1 HypergeometricU[-(2 g2-g1 k-g1 Sqrt[k^2-4 m])/(2 Sqrt[k^2-4 m]),g1,a Sqrt[k^2-4 m]+Sqrt[k^2-4 m] x]+ℯ^(-(k x)/(2)-(1)/(2) Sqrt[k^2-4 m] x) 2 LaguerreL[(2 g2-g1 k-g1 Sqrt[k^2-4 m])/(2 Sqrt[k^2-4 m]),-1+g1,a Sqrt[k^2-4 m]+Sqrt[k^2-4 m] x]}}
```

The solution obtained is not the general solution of the ODE for specific values of parameters $g_{1}=g_{2}=0$:

```wolfram
y[x]/.sol[[1]]/.{g1->0,g2->0}//Simplify
(* Output *)
...
```wolfram
DSolve[eqn/.{g1->0,g2->0},y[x],x]
(* Output *)
{{y[x]->ℯ^((1)/(2) (-k-Sqrt[k^2-4 m]) x) 1+ℯ^((1)/(2) (-k+Sqrt[k^2-4 m]) x) 2}}
```
...
```wolfram
DSolve[{x'[s]==Cos[t[s]],y'[s]==Sin[t[s]],t'[s]==s,x[0]==0,y[0]==0,t[0]==0},{x,y,t},s]
(* Output *)
{{t->Function[{s},(s^2)/(2)],x->Function[{s},Sqrt[π] FresnelC[(s)/(Sqrt[π])]],y->Function[{s},Sqrt[π] FresnelS[(s)/(Sqrt[π])]]}}
```
...
```wolfram
sol=DSolve[(-((192n(1+n)x^5)/(1-x^2)^6)+(2496n(1+n)x^3)/(1-x^2)^5-(8160n^2(1+n)^2x^3)/(1-x^2)^5-(1632n(1+n)x)/(1-x^2)^4+(7824n^2(1+n)^2x)/(1-x^2)^4-(6912n^3(1+n)^3x)/(1-x^2)^4)y[x]+((64x^6)/(1-x^2)^6-(1824x^4)/(1-x^2)^5+(8256n(1+n)x^4)/(1-x^2)^5+(2880x^2)/(1-x^2)^4-(19200n(1+n)x^2)/(1-x^2)^4+(22896n^2(1+n)^2x^2)/(1-x^2)^4-272/(1-x^2)^3+(2208n(1+n))/(1-x^2)^3-(4384n^2(1+n)^2)/(1-x^2)^3+(2304n^3(1+n)^3)/(1-x^2)^3) y'[x]+(-((2016x^5)/(1-x^2)^5)+(9408x^3)/(1-x^2)^4-(19488n(1+n)x^3)/(1-x^2)^4-(3696x)/(1-x^2)^3+(12768n(1+n)x)/(1-x^2)^3-(9408n^2(1+n)^2x)/(1-x^2)^3)y''[x]+((4816x^4)/(1-x^2)^4-(7168x^2)/(1-x^2)^3+(9632n(1+n)x^2)/(1-x^2)^3+616/(1-x^2)^2-(1456n(1+n))/(1-x^2)^2+(784n^2(1+n)^2)/(1-x^2)^2)y'''[x]+(-((2800x^3)/(1-x^2)^3)+(1400x)/(1-x^2)^2-(1400n(1+n)x)/(1-x^2)^2) y''''[x]+((560x^2)/(1-x^2)^2-70/(1-x^2)+(56n(1+n))/(1-x^2))y'''''[x]-(42x y''''''[x])/(1-x^2)+y'''''''[x]==0,y,x]
(* Output *)
{{y->Function[{x},1 LegendreP[n,x]^6+2 LegendreP[n,x]^5 LegendreQ[n,x]+3 LegendreP[n,x]^4 LegendreQ[n,x]^2+4 LegendreP[n,x]^3 LegendreQ[n,x]^3+5 LegendreP[n,x]^2 LegendreQ[n,x]^4+6 LegendreP[n,x] LegendreQ[n,x]^5+7 LegendreQ[n,x]^6]}}
```
...
## Related Guides ▪Equation Solving ▪Calculus ▪Differential Equations ▪Differential Operators ▪Partial Differential Equations ▪Mathematical Data ▪Fractional Calculus ▪Scientific Models ▪Integral Transforms ▪Symbolic Vectors, Matrices and Arrays ▪Solvers over Regions

## History Introduced in 1991 (2.0) | Updated in 1996 (3.0) ▪ 2003 (5.0) ▪ 2014 (10.0) ▪ 2015 (10.3) ▪ 2016 (11.0) ▪ 2019 (12.0) ▪ 2021 (13.0) ▪ 2022 (13.1) ▪ 2022 (13.2) ▪ 2024 (14.1) ▪ 2025 (14.2) ▪ 2026 (15.0)
