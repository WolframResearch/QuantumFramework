...
As we can see, the energy difference between the two eigenlevels (the level splitting) is $2\omega_{x}\sigma_{x}$. A more convenient convention for the Hamiltonian is to write it as $H=1/2\omega_{x}\sigma_{x}$, so that $\omega_{x}$ directly denotes the frequency associated with the level splitting.
...
Solve Schrodinger equation explicitly for with Hamiltonian $H=1/2\omega_{x}\sigma_{x}$ and the initial state $|\psi (0)\rangle =c_{0}|0\rangle +c_{1}|0\rangle $:
