...
Before we start, a few practical notes. This is the Python edition of the book: the narrative and the Wolfram Language computations are identical to the original, but instead of running inside the Mathematica front end, each computation is sent to a free [Wolfram Engine](https://www.wolfram.com/engine/) and evaluated there through the [`wolframclient`](https://reference.wolfram.com/language/WolframClientForPython/) library. You therefore need no paid license and no Mathematica installation to run any of it. The original, fully interactive Wolfram notebook lives in the Wolfram Cloud at [wolfr.am/QIS-Book](https://wolfr.am/QIS-Book), and this edition follows it cell for cell. Evaluate the cells from top to bottom. Although I did my best to make the input cells independent, some variables are defined in earlier cells and used later, so certain cells depend on earlier ones. Be mindful of these dependencies as you run through the document.
...
This interactive, exploratory style is the whole point: change the inputs, run your own variations, and treat the document as a laboratory for your ideas rather than a text to read passively. If you want the full Wolfram notebook experience, the live front end, dynamic manipulation, and a one-click “Make Your Own Copy” in the Wolfram Cloud, open the original at [wolfr.am/QIS-Book](https://wolfr.am/QIS-Book).
...
# evaluated there. You do not need a paid license or Mathematica: the free Wolfram
# Engine is enough. It is free for personal, learning, and pre-production use and
# needs only a (free) Wolfram ID, no payment.
#
#   1) pip install wolframclient
#   2) Download the free Wolfram Engine: https://www.wolfram.com/engine/
#   3) Activate it once by running `wolframscript` and signing in with a free Wolfram
#      ID (this also grants a free Wolfram Cloud Basic account, used by the cloud
#      ResourceFunction["BlochSpherePlot"] cells).
# Works on macOS, Windows, and Linux; the kernel is located automatically below
# (override with the WOLFRAM_KERNEL environment variable if needed).
#
...
import os, glob, shutil, platform, subprocess
from wolframclient.evaluation import WolframLanguageSession
from wolframclient.language import wlexpr

def _find_kernel():
    """Locate a Wolfram kernel on macOS, Windows, or Linux. Works with the free
    Wolfram Engine, Wolfram Desktop, or Mathematica. Returns a path, or None to let
    wolframclient try its own auto-detection. No hard-coded machine-specific paths:
    discovery is delegated to wolframclient and to `wolframscript` on the PATH, with
    a per-OS glob only as a last resort."""
    _sys = platform.system()  # 'Darwin' (macOS), 'Windows', or 'Linux'

    # 1. Explicit override: point WOLFRAM_KERNEL at the WolframKernel(.exe) binary.
    env = os.environ.get("WOLFRAM_KERNEL")
    if env and os.path.exists(env):
        return env

    # 2. wolframclient's built-in per-OS discovery (covers standard Engine / Desktop /
    #    Mathematica installs on all three platforms).
    for _mod in ("wolframclient.utils.environment",
                 "wolframclient.evaluation.kernel.kernelcontroller"):
        try:
            m = __import__(_mod, fromlist=["find_default_kernel_path"])
            p = m.find_default_kernel_path()
            if p and os.path.exists(p):
                return p
        except Exception:
            pass

    # 3. Ask `wolframscript` (on the PATH after any install) where its kernel lives.
    #    This is the most portable route and handles non-standard app names.
    ws = shutil.which("wolframscript") or shutil.which("wolframscript.exe")
    if ws:
        try:
            base = subprocess.run(
                [ws, "-code", "$InstallationDirectory"],
                capture_output=True, text=True, timeout=120
            ).stdout.strip().strip('"')
            if base:
                rel = ("WolframKernel.exe" if _sys == "Windows"
                       else "MacOS/WolframKernel" if _sys == "Darwin"
                       else "Executables/WolframKernel")
    ...
    # 4. Last-resort glob over the standard install locations (newest first).
    if _sys == "Darwin":
        pats = ["/Applications/Wolfram*.app/Contents/MacOS/WolframKernel"]
    elif _sys == "Windows":
        pats = [r"C:\Program Files\Wolfram Research\*\*\WolframKernel.exe"]
    else:  # Linux
        pats = ["/usr/local/Wolfram/*/*/Executables/WolframKernel",
                "/opt/Wolfram/*/*/Executables/WolframKernel"]
    hits = sorted({h for p in pats for h in glob.glob(p)}, reverse=True)
...
try:
    ...
    raise SystemExit(
        "Could not start a Wolfram kernel. Install the FREE Wolfram Engine "
        "(https://www.wolfram.com/engine/), activate it once with a free Wolfram ID "
        "by running `wolframscript`, then re-run. You can also set WOLFRAM_KERNEL to "
        "the full path of the WolframKernel executable. Original error: %r" % (_e,))
...
```
{α/Sqrt[Abs[α]^2 + Abs[β]^2], β/Sqrt[Abs[α]^2 + Abs[β]^2]}
```
...
```
Cos[θ1/2]*Cos[θ2/2] + E^(I*(-ϕ1 + ϕ2))*Sin[θ1/2]*Sin[θ2/2]
```
...
```python
wlshow(r'''ResourceFunction["BlochSpherePlot"][ImageSize -> Small]''', "fig-003")
```

![fig-003](qis-figs/fig-003.png)
*fig-003*
...
```python
wlshow(r'''ResourceFunction["BlochSpherePlot"][ImageSize -> Small, "Labels" -> "Optics"]''', "fig-004")
```

![fig-004](qis-figs/fig-004.png)
*fig-004*
...
```
{Cos[ϕ]*Sin[2*θ], Sin[2*θ]*Sin[ϕ], Cos[2*θ]}
```
...
```python
wlshow(r'''ResourceFunction["BlochSpherePlot"][ImageSize -> Small]''', "fig-005")
```

![fig-005](qis-figs/fig-005.png)
*fig-005*
...
```python
wlshow(r'''ResourceFunction["BlochSpherePlot"][Normalize /@ Eigenvectors[PauliMatrix[1]], 
 ImageSize -> Small]''', "fig-008")
```

![fig-008](qis-figs/fig-008.png)
*fig-008*
...
```python
wlshow(r'''ResourceFunction["BlochSpherePlot"][Normalize /@ Eigenvectors[PauliMatrix[2]], 
 ImageSize -> Small]''', "fig-011")
```

![fig-011](qis-figs/fig-011.png)
*fig-011*
...
```python
wlshow(r'''ResourceFunction["BlochSpherePlot"][Normalize /@ Eigenvectors[PauliMatrix[3]], 
 ImageSize -> Small]''', "fig-013")
```

![fig-013](qis-figs/fig-013.png)
*fig-013*
...
```python
wlshow(r'''Manipulate[ResourceFunction["BlochSpherePlot"][{Cos[\[Phi]] Sin[\[Theta]], 
...
![fig-015](qis-figs/fig-015.gif)
*fig-015, animation: azimuthal angle \(\phi\) swept 0..2\(\pi\) (latitude circle on the Bloch sphere).*
...
```
Im[Subscript[A, 1, 1]] == 0 && Subscript[A, 1, 2] == Conjugate[Subscript[A, 2, 1]] && Im[Subscript[A, 2, 2]] == 0
```
...
```python
wlshow(r'''With[{c1 = RGBColor[0.368417, 0.506779, 0.709798],
   c2 = RGBColor[0.880722, 0.611041, 0.142051]},
 Graphics[{EdgeForm[GrayLevel[0.6]],
   c1, Rectangle[{0, 1}, {1, 2}], c2, Rectangle[{1, 1}, {2, 2}],
   c2, Rectangle[{0, 0}, {1, 1}], c1, Rectangle[{1, 0}, {2, 1}],
   c1, Rectangle[{3.2, 1.25}, {3.9, 1.95}],
   c2, Rectangle[{3.2, 0.05}, {3.9, 0.75}],
   Black, Text[Style["Diagonal elements", 12], {4.1, 1.6}, {-1, 0}],
   Text[Style["Off-diagonal elements", 12], {4.1, 0.4}, {-1, 0}]},
  PlotRange -> {{-0.2, 9}, {-0.3, 2.3}}, ImageSize -> 360,
  AspectRatio -> 1/3.2]]''', "fig-017")
```

![fig-017](qis-figs/fig-017.png)
*fig-017*
...
```
{{0.9999999999999998 + 2.597870184905994*^-17*I, 8.326672684688674*^-17 + 1.1102230246251565*^-16*I}, {8.326672684688674*^-17 - 1.6653345369377348*^-16*I, 1.0000000000000002 - 7.417407635449573*^-18*I}}
```
...
```
{{-0.7927410386514047 - 0.515586493662566*I, 0.23529065178175984 - 0.22443378171466802*I}, {0.0024042948302313225 + 0.32515601264162713*I, 0.9218274827239714 + 0.21095468448023127*I}}
```
...
```
{0.7785232976750408 + 0.5373647858222099*I, 0.17513567781005612 - 0.2728883586623698*I}
```
...
```
{{-0.7927410386514047 - 0.515586493662566*I, 0.0024042948302313225 + 0.32515601264162713*I}, {0.23529065178175984 - 0.22443378171466802*I, 0.9218274827239714 + 0.21095468448023127*I}}
```
...
```
{-0.9825356061472641 - 0.08219754390246464*I, 0.16645424650572632 + 0.012662158166796722*I}
```
...
```
{0.6743387285049275 + 0.18701307510871293*I, 0.4266594776905414 + 0.5729354929427698*I}
```
...
![fig-022](qis-figs/fig-022.gif)
*fig-022, animation: external-field gradient angle \(\theta\) swept 0..2\(\pi\).*
...
```
{Cos[θ/2]/Sqrt[2] + (E^(I*ϕ)*Sin[θ/2])/Sqrt[2], Cos[θ/2]/Sqrt[2] - (E^(I*ϕ)*Sin[θ/2])/Sqrt[2]}
```
...
```
{Abs[1 + E^(I*ϕ)]^2/4, Abs[1 - E^(I*ϕ)]^2/4}
```
...
```
{Cos[θ/2]/Sqrt[2] - (I*E^(I*ϕ)*Sin[θ/2])/Sqrt[2], Cos[θ/2]/Sqrt[2] + (I*E^(I*ϕ)*Sin[θ/2])/Sqrt[2]}
```
...
```
(Subscript[ρ, 1, 1] + Subscript[ρ, 1, 2] + Subscript[ρ, 2, 1] + Subscript[ρ, 2, 2])/2
```
...
```
{c0/Sqrt[2] - (I*c1)/Sqrt[2], c0/Sqrt[2] + (I*c1)/Sqrt[2]}
```
...
```
Subscript[ρ, 1, 1] + Subscript[ρ, 2, 2]
```
...
```
{(1 - Sqrt[rx^2 + ry^2 + rz^2])/2, (1 + Sqrt[rx^2 + ry^2 + rz^2])/2}
```
...
```
{Sqrt[rx^2 + ry^2 + rz^2], ArcTan[rz, Sqrt[rx^2 + ry^2]], ArcTan[rx, ry]}
```
...
```
-((2*Sqrt[rx^2 + ry^2 + rz^2]*ArcTanh[Sqrt[rx^2 + ry^2 + rz^2]] + Log[1/4 - Sqrt[rx^2 + ry^2 + rz^2]/4] + Log[1 + Sqrt[rx^2 + ry^2 + rz^2]])/Log[4])
```
...
Note that the above Gram matrix is proportional to the identity, i.e. $\mathcal{G}=2\mathbb{I}$. Recall that the identity matrix has elements $\mathbb{I}_{ij}=\delta _{ij}$. So the set $\{\sigma _{0},\sigma _{1},\sigma _{2},\sigma _{3}\}$ is Hilbert-Schmidt orthogonal, i.e. $\mathcal{G}_{\mu \nu }\propto \delta _{\mu \nu }$. Note that $\mathrm{Tr}[A^{\dagger }B]$ is called the Hilbert–Schmidt inner product (also denoted as $\langle A,B\rangle $). Since all Pauli matrices and identify matrix are Hermitian ($\sigma _{\mu }^{\dagger }=\sigma _{\mu }$), Hilbert-Schmidt orthogonal for them can be also written as $Tr[\sigma _{\mu }.\sigma _{\nu }]=\mathcal{G}_{\mu \nu }=2\delta _{\mu \nu }$.
...
```
{(1/2 - I/2)*Subscript[c, L] + (1/2 + I/2)*Subscript[c, R], (1/2 + I/2)*Subscript[c, L] + (1/2 - I/2)*Subscript[c, R]}
```
...
```
{(rx + rx*Cos[α] - 2*rx*Cos[θ]^2*Sin[α/2]^2 - 2*ry*Cos[θ]*Sin[α] + 2*Sin[α/2]^2*(rx*Cos[2*ϕ]*Sin[θ]^2 + rz*Cos[ϕ]*Sin[2*θ]) + 2*Sin[θ]*(rz*Sin[α] + 2*ry*Cos[ϕ]*Sin[α/2]^2*Sin[θ])*Sin[ϕ])/2, (ry*(1 + Cos[α]))/2 - ry*Cos[θ]^2*Sin[α/2]^2 + rx*Cos[θ]*Sin[α] - rz*Cos[ϕ]*Sin[α]*Sin[θ] - ry*Cos[ϕ]^2*Sin[α/2]^2*Sin[θ]^2 + rz*Sin[α/2]^2*Sin[2*θ]*Sin[ϕ] + ry*Sin[α/2]^2*Sin[θ]^2*Sin[ϕ]^2 + rx*Sin[α/2]^2*Sin[θ]^2*Sin[2*ϕ], rz/2 + (rz*Cos[α])/2 + rz*Cos[2*θ]*Sin[α/2]^2 + Sin[α]*Sin[θ]*(ry*Cos[ϕ] - rx*Sin[ϕ]) + Sin[α/2]^2*Sin[2*θ]*(rx*Cos[ϕ] + ry*Sin[ϕ])}
```
...
```
{Cos[θ]*(-(ry*Sin[α]) + rz*Cos[ϕ]*Sin[θ]) + Cos[α]*(Cos[θ]*Cos[ϕ]*(rx*Cos[θ]*Cos[ϕ] - rz*Sin[θ]) - ry*Cos[ϕ]*Sin[θ]^2*Sin[ϕ] + rx*Sin[ϕ]^2) + Sin[θ]*(rz*Sin[α]*Sin[ϕ] + Cos[ϕ]*Sin[θ]*(rx*Cos[ϕ] + ry*Sin[ϕ])), Cos[ϕ]*(Cos[θ]*(rx*Sec[ϕ]*Sin[α] + rz*Sin[θ]*Tan[ϕ]) + Sin[θ]*(-(rz*Sin[α]) + Sin[θ]*Sin[ϕ]*(rx + ry*Tan[ϕ])) + Cos[α]*(-(Sin[θ]*(rx*Sin[θ]*Sin[ϕ] + rz*Cos[θ]*Tan[ϕ])) + ry*(Cos[ϕ] + Cos[θ]^2*Sin[ϕ]*Tan[ϕ]))), (rz + rz*Cos[α] + 2*Sin[α/2]^2*(rz*Cos[2*θ] + Sin[2*θ]*(rx*Cos[ϕ] + ry*Sin[ϕ])))/2 + Cos[ϕ]*Sin[α]*Sin[θ]*(ry - rx*Tan[ϕ])}
```
...
```
{(-1/4*I)*Subscript[θ, 2]*Subscript[θ, 3], 0, 0}
```
...
```
{(I/4)*Subscript[θ, 2]*Subscript[θ, 3], 0, 0}
```
...
```python
wlshow(r'''Manipulate[Show[ResourceFunction["BlochSpherePlot"][{{1, 0, 0}, 
...
![fig-048](qis-figs/fig-048.gif)
*fig-048, animation: rotation angle \(\theta\) about z swept 0..2\(\pi\).*
...
```python
wlshow(r'''Manipulate[Legended[ResourceFunction["BlochSpherePlot"][{{0, 0, 1}, 
...
![fig-049](qis-figs/fig-049.gif)
*fig-049, animation: the four discrete pulses X_{\(\pi\)/2}, X_{\(\pi\)}, Y_{\(\pi\)/2}, Y_{\(\pi\)}.*
...
```
{ArcCot[Sqrt[12 - 6*Sqrt[2]]], ArcCot[Sqrt[(319 + 192*Sqrt[2])/97]], ArcCot[Sqrt[3]*Sec[Pi/8]]}
```
...
```
{0.49001086029155794, 0.38505787590347806, 0.49001086029155794}
```
...
```
{{-(Tan[θ/2]/E^(I*ϕ)), 1}, {Cot[θ/2]/E^(I*ϕ), 1}}
```
...
```
{rx, ry*Sqrt[1 - η^2], rz*Sqrt[1 - η^2]}
```
...
```python
wlshow(r'''With[{\[Eta] = .75}, Show[ResourceFunction["BlochSpherePlot"][ImageSize -> 250],
...
```
{(rx - η - ry*Sqrt[1 - η^2])/2, (rx + η + ry*Sqrt[1 - η^2])/2, rz*Sqrt[1 - η^2]}
```
...
```python
wlshow(r'''Legended[With[{\[Eta] = .75}, Show[ResourceFunction["BlochSpherePlot"][ImageSize -> 250],
   ParametricPlot3D[{Cos[\[Phi]] Sin[\[Theta]], 
     Sin[\[Phi]] Sin[\[Theta]] Sqrt[1 - \[Eta]^2], 
     Cos[\[Theta]] Sqrt[1 - \[Eta]^2]}, {\[Theta], 0, \[Pi]}, {\[Phi],
      0, 2 \[Pi]}, Boxed -> False, Axes -> False, PlotPoints -> 25, 
...
```
Quantity[1.0545718176461565*^-34, "Joules"*"Seconds"]
```
...
As we can see, the energy difference between the two eigenlevels (the level splitting) is $2\omega _{x}$. A more convenient convention for the Hamiltonian is to write it as $H=1/2\omega _{x}\sigma _{x}$, so that $\omega _{x}$ directly denotes the frequency associated with the level splitting.
...
![fig-064](qis-figs/fig-064.gif)
*fig-064, animation: rotation phase \(\omega_x t\) swept 0..2\(\pi\).*
...
Solve Schrodinger equation explicitly for with Hamiltonian $H=1/2\omega _{x}\sigma _{x}$ and the initial state $|\psi (0)\rangle=c_{0}|0\rangle+c_{1}|1\rangle$:
...
```
{Cos[(t*ωx)/2]*Subscript[c, 0] - I*Sin[(t*ωx)/2]*Subscript[c, 1], (-I)*Sin[(t*ωx)/2]*Subscript[c, 0] + Cos[(t*ωx)/2]*Subscript[c, 1]}
```
...
![fig-065](qis-figs/fig-065.gif)
*fig-065, animation: drive frequency \(\omega_x\) swept 1..10 (state |0\(\rangle\)).*
...
```
{1/(Sqrt[2]*E^((I/2)*t*ωx)), 1/(Sqrt[2]*E^((I/2)*t*ωx))}
```
...
```
{-((ωx*ωz*(-1 + Cos[t*Ω]))/Ω^2), -((ωx*Sin[t*Ω])/Ω), (ωz^2 + ωx^2*Cos[t*Ω])/Ω^2}
```
...
![fig-066](qis-figs/fig-066.gif)
*fig-066, animation: evolution time t_f swept up to 10 (trajectory traced out).*
...
![fig-067](qis-figs/fig-067.gif)
*fig-067, animation: Gaussian pulse width \(\sigma\) swept.*
...
```
Sqrt[Pi/2]*τ*Ωmax*(Erf[(t - μ)/(Sqrt[2]*τ)] + Erf[μ/(Sqrt[2]*τ)])
```
...
![fig-070](qis-figs/fig-070.gif)
*fig-070, animation: number of random-walk paths = 1,2,3,4.*
...
```
<|"Mean" -> 0.01052, "Variance" -> 19.96512898088981|>
```
...
![fig-077](qis-figs/fig-077.gif)
*fig-077, animation: phase \(\phi\) swept.*
...
Generate the real part electric field, $\mathrm{Cos}[\omega_{0}t+\phi (t)]$ with $\omega _{0}=1$, where the phase follows a one-dimensional Brownian motion:
...
```
(Sqrt[2/Pi]*κ*(κ^2 + ω^2 + ω0^2))/((κ^2 + (ω - ω0)^2)*(κ^2 + (ω + ω0)^2))
```
...
![fig-081](qis-figs/fig-081.gif)
*fig-081, animation: phase-diffusion constant \(\kappa\) swept (linewidth broadening).*
...
Calculate the covariance matrix $\phi _{t}$ and $\phi _{t+\tau }$ which is $\Sigma =\begin{pmatrix}Cov[\phi _{t},\phi _{t}] & Cov[\phi _{t},\phi _{t+\tau }] \\ Cov[\phi _{t+\tau },\phi _{t}] & Cov[\phi _{t+\tau },\phi _{t+\tau }]\end{pmatrix}$ with $\mathrm{Cov}[\ldots ]$ the covariance function:
...
```
(E^(-(κ*(4*t + τ)) - I*(2*t + τ)*ω0)*(1 + E^((2*I)*(2*t + τ)*ω0) + E^(4*t*κ + (2*I)*t*ω0)*(1 + E^((2*I)*τ*ω0))))/4
```
...
![fig-082](qis-figs/fig-082.gif)
*fig-082, animation: first time t swept 0..10 (approach to the stationary correlation).*


As you can see, for sufficiently large time $t$, the two-time correlation function becomes stationary: it no longer depends on $t$ itself, only on the time difference τ. In this long-time limit we have: $lim_{t->\infty }\langle E(t)E(t+\tau )\rangle =1/2e^{-\kappa \, \tau }Cos[\omega _{0}\tau ]$. So the correlations decay exponentially in τ with rate κ, while oscillating at the carrier frequency $\omega _{0}$; which is exactly the same as the mean value of the electric field we had calculated before.
...
Since we can use the usual algebra and calculus of ODE in Stratonovich form, we can write the SDE for the density matrix given the random Hamiltonian above as: $d\rho =-i/2\omega _{x}[\sigma _{x},\rho ]dt-i\sqrt{\gamma }[\sigma _{z},\rho ]\circ dW_{t}$. Let’s write the density matrix as $\rho =1/2(\mathbb{I}+\vec{v}.\vec{\sigma })$ with $\vec{v}$ the Bloch vector $r\{Cos[\phi ]Sin[\theta ],Sin[\phi ]Sin[\theta ],Cos[\theta ]\}$ and find the stochastic dynamical equation of $\{r,\theta ,\phi \}$ in Stratonovich form. As said before, we can use the usual rules of ODE for variable changes.
...
```
StratonovichProcess[{{0, -(ωx*Sin[ϕ[t]]), -(ωx*Cos[ϕ[t]]*Cot[θ[t]])}, {{0}, {0}, {2*Sqrt[γ]}}, {r[t], θ[t], ϕ[t]}}, {{r, θ, ϕ}, {r0, θ0, ϕ0}}, {t, 0}]
```
...
```
<<TemporalData object, ByteCount = 240025120>>
```
...
```python
wlshow(r'''Show[ResourceFunction["BlochSpherePlot"][], 
...
```python
wlshow(r'''Show[ResourceFunction["BlochSpherePlot"][ImageSize -> 250],
...
```
ItoProcess[{{-2*γ*x[t], -2*γ*y[t] - ωx*z[t], ωx*y[t]}, {{-2*Sqrt[γ]*y[t]}, {2*Sqrt[γ]*x[t]}, {0}}, {x[t], y[t], z[t]}}, {{x, y, z}, {x0, y0, z0}}, {t, 0}]
```
...
```python
wl(r'''\[Rho]t = With[{\[Omega]x = 2, \[Gamma] = .2}, ResourceFunction["LindbladSolve"][
...
```
InterpolatingFunction[{{0., 10.}}, {5, 3, 1, {134}, {4}, 0, 0, 0, 0, Automatic, {}, {}, False}, {{0., 0.0001405596799016426, 0.0002811193598032852, 0.0005622387196065704, 0.0008433580794098555, 0.0011244774392131407, 0.003935671037245993, 0.006746864635278843, 0.009558058233311696, 0.012369251831344547, 0.03740118702265717, 0.06243312221396979, 0.0874650574052824, 0.11249699259659503, 0.13752892778790765, 0.18063263518124129, 0.2237363425745749, 0.26684004996790855, 0.30994375736124213, 0.35304746475457577, 0.3961511721479094, 0.4563824643004163, 0.5166137564529232, 0.5768450486054301, 0.637076340757937, 0.6973076329104438, 0.7575389250629507, 0.8177702172154576, 0.8894511622721977, 0.9611321073289376, 1.0328130523856776, 1.1044939974424177, 1.1761749424991577, 1.2478558875558976, 1.3195368326126375, 1.3992962531866582, 1.4790556737606786, 1.558815094334699, 1.6385745149087194, 1.71833393548274, 1.7980933560567605, 1.8778527766307809, 1.9576121972048015, 2.0519074665135744, 2.1462027358223477, 2.240498005131121, 2.334793274439894, 2.429088543748667, 2.5233838130574404, 2.6176790823662133, 2.7119743516749866, 2.804196455727774, 2.896418559780561, 2.988640663833349, 3.080862767886136, 3.1730848719389235, 3.265306975991711, 3.357529080044498, 3.449751184097286, 3.525810921136657, 3.6018706581760287, 3.6779303952154, 3.7539901322547715, 3.8300498692941427, 3.9061096063335143, 3.9821693433728855, 4.058229080412257, 4.137462816723027, 4.216696553033798, 4.295930289344568, 4.375164025655338, 4.454397761966109, 4.533631498276879, 4.61286523458765, 4.69209897089842, 4.764485720767178, 4.836872470635936, 4.909259220504694, 4.981645970373451, 5.054032720242209, 5.126419470110967, 5.198806219979725, 5.271192969848483, 5.350560999234835, 5.429929028621187, 5.509297058007538, 5.58866508739389, 5.668033116780242, 5.7474011461665935, 5.826769175552945, 5.906137204939297, 5.989496074894861, 6.072854944850426, 6.156213814805991, 6.239572684761556, 6.322931554717121, 6.406290424672686, 6.48964929462825, 6.573008164583815, 6.65636703453938, 6.739725904494945, 6.82308477445051, 6.926847528902342, 7.030610283354174, 7.134373037806006, 7.238135792257839, 7.341898546709671, 7.445661301161503, 7.549424055613335, 7.653186810065168, 7.756949564517, 7.860712318968832, 7.964475073420664, 8.068237827872498, 8.17200058232433, 8.275763336776162, 8.379526091227994, 8.483288845679827, 8.587051600131659, 8.690814354583491, 8.794577109035323, 8.898339863487156, 9.002102617938988, 9.10586537239082, 9.209628126842652, 9.313390881294485, 9.417153635746317, 9.520916390198149, 9.624679144649981, 9.728441899101814, 9.832204653553646, 9.90980474795493, 9.954902373977465, 10.}}, {{{{0.8535533905932737 + 0.*I, 0.35355339059327373 + 0.*I}, {0.35355339059327373 + 0.*I, 0.14644660940672627 + 0.*I}}, {{2.7755575615628914*^-17 + 0.*I, -0.14142135623730948 + 0.7071067811865475*I}, {-0.14142135623730948 - 0.7071067811865475*I, 3.469446951953614*^-18 + 0.*I}}}, {{{0.8535533626541939 + 0.*I, 0.35353351357027296 + 0.00009938510714924098*I}, {0.35353351357027296 - 0.00009938510714924098*I, 0.1464466373458061 + 0.*I}}, {{-0.00019877022937941913 + 0.*I, -0.1414134054281343 + 0.70706697125937*I}, {-0.1414134054281343 - 0.70706697125937*I, 0.00019877022937942954 + 0.*I}}}, {{{0.8535533067776115 + 0.*I, 0.35351363766477256 + 0.00019876461108998724*I}, {0.35351363766477256 - 0.00019876461108998724*I, 0.14644669322238846 + 0.*I}}, {{-0.0003975292372591921 + 0.*I, -0.14140545506593413 + 0.7070271077046251*I}, {-0.14140545506593413 - 0.7070271077046251*I, 0.00039752923725918515 + 0.*I}}}, {{{0.8535531391525986 + 0.*I, 0.35347388808864677 + 0.00039751239747961404*I}, {0.35347388808864677 - 0.00039751239747961404*I, 0.1464468608474015 + 0.*I}}, {{-0.000795024794955773 + 0.*I, -0.1413895552354587 + 0.7069472733461973*I}, {-0.1413895552354587 - 0.7069472733461973*I, 0.0007950247949557593 + 0.*I}}}, {{{0.8535528597902043 + 0.*I, 0.3534341429820199 + 0.0005962377107366286*I}, {0.3534341429820199 
... (output truncated)
```
...
```python
wlshow(r'''Legended[Show[ResourceFunction["BlochSpherePlot"][ImageSize -> 200],
...
```python
wlshow(r'''Manipulate[state = ResourceFunction["LindbladSolve"][
   1/2 \[Omega]x PauliMatrix[1], {Sqrt[\[Gamma]] PauliMatrix[3]}, 
...
![fig-092](qis-figs/fig-092.gif)
*fig-092, animation: Lindblad jump rate \(\gamma\) swept 0..1 (coherent circle -> decohering spiral).*
...
```
ItoProcess[{{(-2*γ*(-1 + r[t]^2)*Sin[θ[t]]^2)/r[t], γ*(-3 + 2/r[t]^2)*Sin[2*θ[t]] - ωx*Sin[ϕ[t]], -(ωx*Cos[ϕ[t]]*Cot[θ[t]])}, {{-2*Sqrt[γ]*Cos[θ[t]]*(-1 + r[t]^2)}, {(-2*Sqrt[γ]*Sin[θ[t]])/r[t]}, {0}}, {r[t], θ[t], ϕ[t]}}, {{r, θ, ϕ}, {r0, θ0, ϕ0}}, {t, 0}]
```
...
```
<<TemporalData object, ByteCount = 2403520>>
```
...
```
Video["https://www.wolframcloud.com/obj/246ac6c5-b0df-4ab5-bed3-4cc226d5e0bd", Appearance -> Automatic, AudioOutputDevice -> Automatic, AudioTrackSelection -> {}, PlaybackSettings -> <||>, SoundVolume -> Automatic, SubtitleTrackSelection -> {}, VideoTrackSelection -> {1}]
```
...
```python
wl(r'''stickSwitch = With[{bs = ResourceFunction["BlochSpherePlot"][]}, 
   Show[bs, 
      Graphics3D[
...
```
Video["https://www.wolframcloud.com/obj/c04bcfb5-3a1b-4ae2-817a-15c3cbe87674", Appearance -> Automatic, AudioOutputDevice -> Automatic, AudioTrackSelection -> {}, PlaybackSettings -> <||>, SoundVolume -> Automatic, SubtitleTrackSelection -> {}, VideoTrackSelection -> {1}]
```
