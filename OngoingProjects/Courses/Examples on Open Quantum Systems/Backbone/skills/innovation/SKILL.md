...
"Refuted" and "known" are different deaths: one means the physical reasoning failed, the other that it succeeded and arrived somewhere already occupied. Both stay on the ledger; a rediscovery with a citation is evidence about the search's calibration, not an embarrassment to hide. A refuted death is further typed by how it died: logical (algebra, dimensional analysis, a limiting case) or empirical (a computation or data). An exclusion on normative or scope grounds is a scope choice, recorded so the pass's coverage stays auditable, and never booked as a kill.
...
The expectation field is the surprise detector: the model under-recognizes its own surprises, so the prediction must be on record for a miss to be mechanical to spot, and a corrected wrong expectation is historically where the best statements have come from. Where a claim has a natural probabilistic outcome, the expectation may be quantitative: an elicited prior, the computed posterior, and a signed shift that counts as a surprise only when it crosses a decision threshold stated alongside the prior; everywhere else the expectation stays a sentence. When several falsifiers compete for the cheapest slot, compare discriminating power per unit cost and pick the test that eliminates the most candidates per unit spent; sequential testing with explicit error control is reserved for noisy statistical tests, never needed for exact decisions. The ledger is the evidence record and may be as detailed as it needs to be; compression is the backbone's discipline, not the ledger's.

## The five moves

**Move 1: variation sweep with priced expectations.** Enumerate what the backbone makes mechanical: each root weakened in each physically meaningful direction, each pair's corner models, regimes and crossings the page does not price (long times, strong coupling, low efficiency, and their intersections, where idealized analyses stop because their idealization does). Order the sweep contradiction-first: pairs of roots in tension, where satisfying one presses against the other, come before free single-root weakenings, because tension is where routine analysis stops and unpriced corners concentrate. Expectation first, then the test.

**Move 2: literature mapping on the graph.** Runs AFTER the generative moves and the cheap kills (see the staging below), in a dual role: attach every published question found to the backbone node or edge it interrogates, killing rediscoveries with their citations, and surface the cells left over. A cell with nothing attached is **unmapped**, never "unaddressed": the search may have missed terminology, the result may live as a lemma inside a paper about something else, the databases have edges, or the cell is a corollary nobody thought worth stating. The honest product is "no close prior found under the recorded protocol", with the protocol written down. Before mapping starts, three things are fixed in writing: an eligibility criterion for what counts as attachable to a cell; the attachment relations a paper may bear to a node or edge (states it, proves it, weakens it, assumes it, contradicts it), with each used relation illustrated by one attached example once mapping is under way; and the blank-cell rule, that an unmapped cell means nothing beyond the kind of search actually run, so the protocol names that kind. Piloting the scheme on the first few papers may broaden a relation or a cell reading; it never multiplies categories. A cell where evidence attached but does not settle the question is typed by why it falls short: insufficient or imprecise, biased, inconsistent, or not the right information. Retrieval hierarchy: the Undermind connector where available (call get_orientation first; deep searches take minutes and are worth it); otherwise arXiv, Crossref, and Semantic Scholar with terminology variants; then citation chaining from the closest papers found; and when coverage is still thin, a recorded limitation instead of a confident absence.

**Move 3: meaning-split arbitrage.** The backbone's definitional discipline splits load-bearing words into inequivalent meanings (deadline versus race, pathwise versus mean versus passage). For each split, determine which meaning the published claims silently assume; the other meaning's version of the claim is a candidate, and this move-type has produced real papers. When a split is found, also check the four ways the two meanings can coexist in one arrangement, separated in time, in space, in scale, or by condition; each coexistence that survives a moment's physical scrutiny is its own candidate regime.
...
A dictionary that maps only vocabulary and yields no falsifiable consequence dies immediately; that filter is what separates transplant from metaphor. Before its consequence is trusted, the dictionary passes a structural audit: it is one-to-one on the objects it pairs; it carries relations rather than attributes (shared structure, not shared adjectives); and the new testable consequences are generated mechanically, as source statements pushed through the dictionary, each admitted only if it connects to the mapped structure rather than hanging from a single translated term.
...
5. Deep literature and citation-chain searches on serious survivors only; fill their survivor fields. For a survivor linking two notions, run the two-node search: enumerate the intermediate terms connecting them and search each link separately. The prior-art verdict quotes the closest passage found, so "known" carries its evidence and "no close prior" shows the nearest miss.
6. Independent verification of finalists (a route sharing nothing with the route that found them).

The requirement is coverage, not mortality: every candidate must face the strongest affordable falsifier relevant to its claim. A pass where everything survives is a diagnostic to investigate, not a quota to correct; and manufacturing harshness (weak candidates bred to die, minor qualifications booked as deaths) is the mirror-image failure and equally disqualifying. A third diagnostic sits beside these two: the near-duplicate rate. Count how many candidates in a pass restate one another; a sweep that keeps regenerating the same claims indicts the enumeration exactly as an all-survivors pass indicts the tests, and the remedy is a different enumeration axis, not more volume. All three diagnostics work only because the ledger records every outcome: a rare survivor is interpretable solely against the full record of what was tried.
...
- A self-assigned score is not a status: the model's own rating of a candidate's novelty or significance is evidence on no axis; only the recorded search moves prior art, and only collisions move validity.
