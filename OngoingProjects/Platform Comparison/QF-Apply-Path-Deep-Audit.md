...
**Environment.** Apple M2 Pro, Mathematica 15.0, paclet 2.0.0 loaded from the working tree (`PacletDirectoryLoad`), repo HEAD `6f953ad5`. All kernel citations are `QuantumFramework/Kernel/<path>:line` at that SHA. All scripts live in `scripts/apply0*.wls` next to this document and emit `RESULT|label|value` lines; timings are `AbsoluteTiming` wall times in ms. Standard benchmark: $n = 12$ qubits, 3 layers of $H^{\otimes n}$, a CNOT chain, and $R_Z(0.3)^{\otimes n}$, 105 gates total, applied to the register $|0\rangle^{\otimes 12}$.
...
|---|---|---|---|---|
| 1 | **Cache machinery repair** (fix D): per-object parametric flag, no write-on-hit, optionally Association-backed memo with eviction | small (the 8 dispatch SubValues) | low: output verified identical; keep the flag as escape hatch | warm apply $3.7\times$ ($832 \to 226$ ms); also fixes the $G^2$ depth scaling and the 25-50 MB/circuit leak |
...
Items 1 and 4 are safe immediate wins; item 2 is the headline competitive fix; item 3 falls out of item 2's compile step almost for free. Interim user-side guidance, today, no kernel change: set `Wolfram`QuantumFramework`PackageScope`$QuantumFrameworkPropCache = False` after a warm-up application for repeated numeric circuit evaluation ($3.7\times$), prefer rebuilding circuits over the `"Parameters"` substitution idiom in numeric sweeps, and never pass `QuantumState[Table[0, {n}], 2]` as a register (use `QuantumState["Register"[n]]`).
