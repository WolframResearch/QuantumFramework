...
2. **Stabilizer simulation** of an identical Clifford gate stream, now reported phase-resolved (Section 3.2; full matrix in the companion `QF-Stabilizer-vs-Packages.md` §2). At $n=1000$, $2\times10^4$ gates: QuantumFramework's compiled tableau **kernel** is within $4$-$7\times$ of scalar Stim ($4.45$ vs $1.01\,\mathrm{ms}$) and $24\times$ faster than QuantumClifford.jl's gate loop; the **end-to-end user path** (which also encodes the circuit and materializes a canonical tableau object) is $11$-$16\times$ behind Stim's fast-I/O pipeline and $1.9\times$ ahead of QC.jl ($58.6$ vs $\approx 5.5$ vs $\approx 110\,\mathrm{ms}$). No single ratio is honest without naming those boundaries: a per-gate Python-driven Stim pipeline is $27\times$ *slower* than QF end to end. The cumulative speedup over the pre-rework implementation is $266\times$ to $1095\times$. QuantumFramework's stabilizer results are **numerically cross-validated against both Stim and QuantumClifford.jl** by its own test suite. **[BENCH]/[SRC]** *(Two prior editions of this item were wrong in opposite directions; Section 3.2 records both errors.)*
...
The shape after June 2026: QuantumFramework's value remains the symbolic object model (exact algebra, free parameters, qudits, ZX, second quantization, phase space, basis reconciliation) and seamless Wolfram-Language integration, but the performance story is no longer uniform. The **stabilizer engine became usable at scale** (a thousand qubits and $2\times10^4$ gates in tens of milliseconds, ahead of QC.jl at large $n$; the compiled kernel itself is within $4$-$7\times$ of scalar Stim, with most of the remaining end-to-end time spent on object boundaries rather than simulation, Section 3.2). The **state-vector path is still behind Aer/Cirq** ($\sim 10^2\times$), but the gap narrowed materially in mid-June 2026: the property-cache refactor (Section 12.2) removed the per-access "memo bookkeeping tax" that had been the dominant cost, taking the warm 12-qubit apply from $836\,\mathrm{ms}$ to $273\,\mathrm{ms}$. What remains is genuine numeric work (dense contraction in sparse containers) plus per-apply re-assembly; the one large win still on the table, a packed numeric fold, is prototyped at $178\times$ but not yet shipped. Open-system work is competitive on small systems already.
...
- **Threading and SIMD:** all runs single-process. Aer can multithread; QuantumFramework's kernels are single-threaded WL. Note that on this Apple-silicon machine the installed Stim wheel is the scalar `march=polyfill` build (the `sse2`/`avx2` binaries are x86-only), so **Stim's Section 3.2 numbers involve no vector instructions at all** and are a lower bound on what it does on x86. This is part of the honest picture, not a controlled variable.
- **Tolerance on the QF column.** Absolute figures drift with machine load: across 10 fresh kernels $n=1000$ ranged $53.0$-$59.3\,\mathrm{ms}$, so treat the QF column as good to about $10\%$, not to three digits. Sweeping all three sizes in one kernel is *not* a source of error (checked: `scripts/qf_final_all.wl` run as-is reproduces $24.8$-$25.0$ and $54.0$-$55.0\,\mathrm{ms}$ at $n=500$/$1000$, indistinguishable from one-size-per-kernel runs, with `MemoryInUse[]` flat across the sweep). On a quiet machine the whole QF column runs $\sim\!8\%$ faster than published.
...
- **Language-boundary discipline (added 2026-07-20).** When a compiled engine is driven from a scripting language, the harness must hand it the *whole workload in one call*. Driving it one operation per call measures the foreign-function boundary instead. This defect produced a $\sim\!10\times$ error in the Stim column of Section 3.2 for a month. **Diagnostic:** compute cost per operation and check how it moves with problem size. If the engine's algorithm says cost per operation must grow with $n$ and the measurement is flat, the measurement is bounded by call overhead, not by the engine. **Boundary-symmetry discipline (added later the same day):** batching is necessary but not sufficient. Every timed region must do the same work on every engine: circuit construction in or out on both sides, output materialization in or out on both sides. The first correction of the Stim column violated this (it charged QF for materializing a canonical tableau while Stim returned nothing) and overstated the gap by $\sim\!8\times$. The cure is phase-resolved timing (`scripts/bench_phases_*`), after which any ratio must name its boundaries to exist at all. All scripts in `scripts/` were re-audited against this test on 2026-07-20:

| Script / path | Structure | Verdict |
|---|---|---|
| `bench_stim_all.py`, `bench_stim_full.py`, `bench_stim_shared.py`, `bench_py.py:bench_stim` | was one `s.h()`/`s.cnot()` call per gate | **Defective, fixed.** Per-gate cost was flat at $479$-$520\,\mathrm{ns}$ for $n=100$-$1000$ (and stayed flat out to $n\approx2000$). All four now build one `stim.Circuit` and call `sim.do()` once |
| `bench_py.py:bench_statevector` (Qiskit Aer, Cirq) | one `sim.run(qc)` / `simulate(c)` for the whole circuit | **Sound.** Already batched. Per-gate cost rises steeply with $n$ ($16.3 \to 59.1\,\mu\mathrm{s}$ from $n=12$ to $16$), the opposite of the flat signature. One caveat: circuit construction and simulator construction sit *inside* the timed region, inflating Aer by $\sim\!20\%$ at $n=12$ and $\sim\!6\%$ at $n=16$. Immaterial against a $10^2\times$ gap, but it flatters QF, not Aer |
| `bench_py.py:bench_qutip_lindblad` | one `qt.mesolve()` for the whole trajectory | **Sound.** The 201-point time grid is integrated inside QuTiP; no per-step Python call |
| `bench_qc_full.jl`, `bench_qc.jl` | per-gate `apply!` in a Julia loop | **Sound.** Native Julia calling native Julia: there is no foreign-function boundary to pay, which is why this column reproduced ($108.7$ vs $112.9\,\mathrm{ms}$) while the Stim column did not |
| `qf_final_all.wl`, `qf_bench_full.wl` | `ps["ApplyCircuit", specs]`, spec conversion outside the timed region | **Sound**, and the reason batched Stim is the apples-to-apples comparison: both hand the whole stream over in one call |
...
> **Correction (2026-07-20).** The Stim column below was wrong, and wrong in QuantumFramework's favour by roughly an order of magnitude. The harness `scripts/bench_stim_all.py` drove Stim **one gate per Python call**, so it timed the Python-to-C++ call boundary instead of the tableau engine. The diagnostic tell: per-gate cost was flat at $479$/$511$/$520\,\mathrm{ns}$ across $n=100$/$500$/$1000$, and a real tableau update is $O(n)$ per gate, so it cannot be flat. Handing Stim the whole stream as one `stim.Circuit` via `sim.do(circuit)` gives $23.6$/$33.0$/$47.6\,\mathrm{ns}$ per gate, which does scale with $n$. The Stim figures below are the corrected batched measurement. The QF and QuantumClifford.jl columns were re-run at the same time and reproduce ($57.1$ and $108.7\,\mathrm{ms}$ at $n=1000$), so they stand as published: QC.jl is native Julia with no foreign-function boundary in its gate loop and never had the defect.

| $n$ / gates | QF original | **QF current** (`ps["ApplyCircuit"]`) | Stim `TableauSimulator` | QuantumClifford.jl `MixedDestabilizer` |
|---|---:|---:|---:|---:|
| 100 / 2000 | $681\,\mathrm{ms}$ | $2.6\,\mathrm{ms}$ | $0.047\,\mathrm{ms}$ | $1.08\,\mathrm{ms}$ |
| 500 / 10000 | $15{,}836\,\mathrm{ms}$ | $26.2\,\mathrm{ms}$ | $0.330\,\mathrm{ms}$ | $23.1\,\mathrm{ms}$ |
| 1000 / 20000 | $63{,}469\,\mathrm{ms}$ | $58.0\,\mathrm{ms}$ | $0.953\,\mathrm{ms}$ | $108.4\,\mathrm{ms}$ |

(QF measured at `6f953ad5`; re-verified at HEAD `02cc92d9` on 2026-06-12: $3.6$ / $25.5$ / $57.0\,\mathrm{ms}$, and again on 2026-07-20: $2.4$ / $25.4$ / $57.1\,\mathrm{ms}$, within run-to-run noise. Stim re-measured 2026-07-20, v1.15.0, `march=polyfill`.)

**Reading it.**
- The cumulative speedup over the pre-rework implementation is **$266\times$ / $606\times$ / $1095\times$**. That figure compares QuantumFramework against its own earlier self and is unaffected by the Stim correction.
- **Do not divide these columns into a single ratio.** The QF column is the end-to-end user path (constructor + circuit encode + kernel + canonical materialization); the Stim column is the batched engine call alone. Dividing them mixes boundaries, which is exactly the mistake two prior editions of this section made, in opposite directions (first "$2.5$-$5.3\times$" from a Stim harness that timed Python call overhead; then "$50$-$85\times$" from charging QF for materialization Stim was never asked to do). The phase-resolved measurement (2026-07-20, `scripts/bench_phases_*`) separates the work: at $n=1000$ QF's compiled kernel runs $4.45\,\mathrm{ms}$ for the $2\times10^4$ gates ($0.22\,\mu\mathrm{s}$/gate, flat in $n$) against Stim's $1.01\,\mathrm{ms}$ ($4.4\times$; $7.3\times$ at $n=100$, narrowing with $n$), while ingest + constructor + pack + materialize account for the other $\sim\!54\,\mathrm{ms}$ of QF's total. End to end with fast I/O on both sides the factor is $11$-$16\times$; if Stim must emit forward stabilizers it is $2.7\times$; a per-gate Python-driven Stim pipeline is $27\times$ slower than QF. Full matrix: `QF-Stabilizer-vs-Packages.md` §2.
- **QuantumFramework beats QuantumClifford.jl at $n=1000$** by $1.9\times$ while roughly matching it at $n=500$. This claim survives the correction and was independently reproduced.
- **The QF column is itself a dated snapshot.** The stabilizer kernel has moved 5 commits past the cited anchor, touching the gate hot paths; re-running the harness at current HEAD gives $2.44$ / $25.9$ / $56.1\,\mathrm{ms}$, so QF is now faster than tabled and the $n=1000$ ratio is nearer $59\times$. A full re-baseline of all three engines at one HEAD has not been done.
- In absolute terms $2\times10^4$ gates on $1000$ qubits in $58\,\mathrm{ms}$ is fast, and the stabilizer formalism makes a problem this size easy for any correct implementation. Where the time actually goes ($n=1000$): $38\%$ materializing the canonical object, $25\%$ encoding gate specs, $18\%$ constructor + packing, $12\%$ dispatch residual, $7.6\%$ simulating. The engine is not the bottleneck; the object boundaries are, and a caller who stays inside the tableau amortizes them. A former validity caveat is gone: the sparse-tableau fallback above $n=1000$ was fixed at `253afdfb` (2026-07-20), and the compiled fold now runs to at least $n=4000$ at $0.22$-$0.50\,\mu\mathrm{s}$/gate, kernel ratio to scalar Stim settling near $3.5$-$4\times$; the $O(n^2)$ boundary charge (materialize, pack) increasingly dominates the end-to-end time instead (companion doc §2).
...
So the prior audit's headline recommendations have been executed. The audit's Tier-3 performance items (a packed machine-word representation and a native-code kernel) **have now also shipped**, in the June 2026 rework (Section 12.1): the tableau bits live in packed 62-bit words, and a single compiled-to-C routine executes whole Clifford circuits. Section 3.2 shows the result: ahead of QC.jl at $n=1000$; the compiled kernel is within $4$-$7\times$ of scalar Stim, with the remaining end-to-end gap living in object boundaries rather than the engine. The remaining genuine throughput item is constant-cost frame sampling (next section).
...
- ~~Bit-packed performance kernels~~ : **closed** by the June 2026 rework (Section 12.1). Phase-resolved (Section 3.2): the compiled kernel is within $4$-$7\times$ of scalar Stim (both sides measured with no vector unit engaged, `march=polyfill`); the larger end-to-end factor is circuit encoding, packing, and canonical materialization, not the kernel.
...
- **QEC / stabilizer simulation** : since June 2026 QuantumFramework's `PauliStabilizer` is a real option: cross-validated against Stim and QC.jl, and ahead of QC.jl at $n=1000$ on raw Clifford streams (kernel within $4$-$7\times$ of scalar Stim; end-to-end $11$-$16\times$ with fast I/O, boundary-dependent), with symbolic signs, qudit support, and the Wolfram environment around it. Choose Stim when you need **bulk shot sampling** (Pauli frames) or **detector error models** for decoders; choose QC.jl for its QEC code library. **[BENCH]**
...
Stim TableauSimulator        n=100: 0.047     n=500: 0.330     n=1000: 0.953   (batched sim.do(circuit), engine call only; phase-resolved rows in scripts/bench_phases_stim.py)
...
- Aer exploits threads; QuantumFramework is single-threaded WL. Stim as measured here is scalar (`march=polyfill`, no SIMD) and single-threaded. The gaps in Section 3 are "as shipped, single machine," not a controlled-thread comparison.
...
### 12.1 Stabilizer engine: a $266$ to $1095\times$ speedup over its own former self (shipped)
...
2. *Run the whole circuit in one native loop.* The remaining cost was the per-step evaluator overhead itself. The entire gate loop was translated once into machine code (via WL `Compile` to C, built lazily and reused), so `ps["ApplyCircuit", specs]` hands the packed tableau and the whole encoded circuit to a single native routine: $0.22$ to $0.27\,\mu\mathrm{s}$ per gate, within $4$-$7\times$ of the scalar Stim build's engine call on the same streams ($0.025$-$0.050\,\mu\mathrm{s}$/gate). One edition of this document called that "Stim's own regime", a later one denied it by comparing against the wrong boundary; the phase-resolved Section 3.2 settles it: same order of magnitude, small constant behind. The same round gave the register constructor its closed form (destabilizers $X_i$, stabilizers $Z_i$, signs $+1$, assembled directly), moved single-qubit measurement onto the packed words, and vectorized the Pauli-letter display (a letter is just $x + 2z$ indexed into $\{I, X, Z, Y\}$).
