...
- **Phase 7.3 — TODO.** Replace the non-Pauli QMO fallback with a real `StabilizerFrame` decomposition. When QMO basis vectors have low stabilizer rank (typical for T-rotated or Clifford-rotated states), decompose them into `Σᵢ cᵢ |sᵢ⟩` and route per-component through the tableau. Cost: `O(rank · n²)` instead of `O(2ⁿ)`. Also: a `qc[ps]` ⇒ `QuantumChannel`-on-mixed-state form once Phase 8 (Yashin25) introduces a "stabilizer mixed state" type. Effort: ~250 LOC + ~20 tests. Tracked here, not in [A](#a-partial-implementations-work-but-incomplete).
