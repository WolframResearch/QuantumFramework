...
> Function-by-function reference for the 10 public symbols + the property/method dispatch on the three head types (`PauliStabilizer`, `StabilizerFrame`, `GraphState`). Every code example is verified by [`verify-API.wls`](verify-API.wls) — `wolframscript -f Documentation/Stabilizer/verify-API.wls`.
...
**Re-verify:** `wolframscript -f Documentation/Stabilizer/verify-API.wls`
...
```bash
wolframscript -f Documentation/Stabilizer/verify-API.wls
```
